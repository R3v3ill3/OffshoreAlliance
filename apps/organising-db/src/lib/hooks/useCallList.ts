...
export function useCallLists(campaignId: number | string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/campaigns/${campaignId}/call-lists`)
...
export function useCallList(campaignId: number | string, listId: number | string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/campaigns/${campaignId}/call-lists/${listId}`)
...
export function useCallListItems(campaignId: number | string, listId: number | string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/campaigns/${campaignId}/call-lists/${listId}/items`)
...
export function usePopulateCallList(campaignId: number | string, listId: number | string) {
  ...
  return useMutation({
    mutationFn: async (filters: Record<string, unknown>) => {
      const res = await fetch(`/api/campaigns/${campaignId}/call-lists/${listId}/populate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
