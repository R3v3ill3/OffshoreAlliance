...
export default function WatchlistPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <MobilisationTabs current="/mobilisation/watchlist" />
        ...
        <ul className="divide-y rounded-lg border">
          {(contractors.data ?? []).map((row) => (
            <li key={row.watch_id} className="flex items-center justify-between gap-3 p-3 text-sm">
              <div>
                <Link className="font-medium underline" href={`/mobilisation/contractors/${row.watch_id}`}>{row.canonical_name}</Link>
        ...
        <ul className="divide-y rounded-lg border">
          {(vessels.data ?? []).map((row) => (
            <li key={row.vessel_id} className="flex items-center justify-between p-3 text-sm">
              <div>
                <Link className="font-medium underline" href={`/mobilisation/vessels/${row.vessel_id}`}>{row.name}</Link>
                <div className="text-xs text-muted-foreground">
