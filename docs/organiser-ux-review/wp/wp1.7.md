...
Applied on `feat/oux-wp1.7-guides-hints` (Fable, 2026-09-10) on top of `fdf4626` (both migrations applied to dev). Input: §7 verifier run 2 — the "Got it" test times out with "element is outside of the viewport" on every retry while the badge-click test and the verifier's own screenshot script pass.

**Why the button was off-screen, and why a single scroll does not fix it.** The callout is positioned next to its anchor, so when the hinted tile is below the fold the copy and "Got it" are below the fold with it (Radix collision handling shifts content within the viewport along its own axis; it cannot move an off-screen anchor). The first spec run of this round (a single `scrollIntoViewIfNeeded()` on the anchor) made "Got it" pass and the badge test fail at **Cancel**, also "outside of the viewport". The trace (`test-results/…/trace.zip`, frames 4490–4623 ms) shows the mechanism: the campaign summary above the tiles is `sticky` and **collapses when its sentinel leaves the viewport** (`campaign-wall-chart.tsx` `isSummaryStuck`, lines 272–286), with scroll anchoring disabled on it and on the units container (lines 1465–1469, 1618–1624). So a scroll that carries the sentinel across the viewport edge is followed by a ~190 px layout shift of everything below the summary: the badge centred at 360 px ends up at ~170 px, behind the stuck bar; Playwright's click hits the bar, retries with an end-alignment, the sentinel comes back, the summary re-expands, and the same shift pushes the just-opened rating popover below the fold. The verifier's script passed by racing that re-layout. On a 650–720 px viewport (a 1366×768 laptop) a one-shot `scrollIntoView({ block: "center" })` in the product would land the hinted tile behind the stuck bar in the common case (first row of the first unit), so "call it once" as briefed is not enough — recorded as a deviation below.

| # | Item | Change |
|---|---|---|
| 1 (product) | Bring the anchor into view when the hint appears — a hint whose target is off-screen is useless, which is why this is product, not test. | `first-use-hint.tsx`: an effect keyed on `visible`, guarded by `scrolledRef` so it runs once per mount and never while hidden. It does nothing if the anchor is already fully within the viewport **and uncovered** (`elementFromPoint` at its centre is the anchor or the callout — the page header and the stuck summary both overlay tiles). Otherwise `scrollIntoView({ block: "center" })`, smooth unless `prefers-reduced-motion: reduce`, then a `requestAnimationFrame` loop waits for the anchor's `top` to be unchanged for 6 frames (cap 120) and re-checks; if the sticky re-layout moved it behind the bar or off-screen, one instant re-centre, at most two corrections (`MAX_SCROLL_ATTEMPTS = 3`). Cancelled on hide/unmount. **Deviation from the brief:** one scroll plus bounded corrections instead of exactly one call, for the reason above. Also `worker-tile.tsx`: `data-worker-name={displayName}` on the tile root, for the spec (item 2). |
| 2 (spec) | `wall-chart.spec.ts` | (a) `openWallChartWithHint` waits for the anchor, then the callout, then gives the product effect up to 3 s and prints `[hint] anchor clear before the spec scrolled: yes/no` — evidence for a preview that carries item 1, not an assertion, so the spec still passes on one that does not (this preview prints `no`). Then `bringAnchorIntoView`: an explicit `scrollIntoView({ block: "center" })` (not `scrollIntoViewIfNeeded()` — "if needed" is geometric, and a badge behind the stuck bar is inside the viewport rect, so the second run of this round never moved it again and both tests failed in the helper), accepted only when the anchor is fully in the viewport and uncovered on two checks 150 ms apart, at most three scrolls. The badge test calls it again before clicking the badge, or Playwright's own retry scroll moves the popover off-screen. (b) The "Got it" negatives (`workerSheet` count 0, badge `aria-expanded="false"`) are asserted **after** `expectDismissalPersisted` — a settled positive that takes a round trip — not the instant after the click, when "not open yet" would pass as "did not open". (c) Worker name: `data-worker-name` on the tile root when the build carries it, else the text of the `line-clamp-2` span beside the cumulative dot inside the tile button (`worker-tile.tsx`, `{displayName}`); no longer parsed out of the button's `title` up to the first ". ", which cut a name with initials short. |
| A (advisory) | Late successful write after a background refetch. | `use-first-use-hint.ts`: `onSuccess` re-applies the entry with the same `setQueryData` `dismiss()` uses (`addToCache`). The comment and fix round 1 finding 8's claim ("a failed write … must never … come back mid-session") were wrong: `staleTime: Infinity` does not survive the bare `queryClient.invalidateQueries()` calls after session recovery (`providers.tsx:67, 312, 352`, `session-recovery.ts:540`), which refetch every active query and replace the cached list with the table's. A late **successful** write now wins over a refetch that raced it; a **failed** write's cached entry lasts only until the next such invalidation — the hint can then come back in the same session, and will next session regardless. Recorded as the corrected behaviour. |
| B, C, E (advisories) | Known limits, no change. | Recorded by the reviewer's letters; the implementer did not have the reviewer's text and has not restated it — the orchestrator holds the one-line descriptions. No code changed for them in this round. |

**Files changed (all under `apps/organising-db/`):** `src/components/hints/first-use-hint.tsx`, `src/components/campaigns/wall-chart/worker-tile.tsx` (+1 attribute), `src/lib/hints/use-first-use-hint.ts`, `tests/e2e/wall-chart.spec.ts`. No migration, no dependency, no database contact, no local app.

**Gates (from `apps/organising-db`):** `pnpm exec eslint` on the four touched files → exit 0, no findings. `pnpm test` → 76 files, 1029 tests passed. `pnpm exec tsc --noEmit -p tsconfig.json` → exit 0. `pnpm build` → exit 0 (route manifest printed, no errors). Credential-less `pnpm e2e` → 14 skipped, exit 0 (hint tests #12, #13).

**Credentialled `wall-chart.spec.ts` against the current preview `https://offshore-alliance-87n5xm5cc-reveille-strategy.vercel.app` (`0617a12`, no product change of this round on it):**

- Run 1 (single `scrollIntoViewIfNeeded()`): 3 passed, 1 failed — "Got it" **passed**; badge test failed at Cancel, "element is outside of the viewport" (the sticky-summary finding above).
- Run 2 (settle loop still using `scrollIntoViewIfNeeded()`): 2 passed, 2 failed in `bringAnchorIntoView` ("the hinted tile must be in the viewport and uncovered after scrolling"); screenshot shows the callout drawn over the stuck bar, the badge behind it.
- Runs 3, 4 and 5 (explicit centre scroll, final spec): **4 passed** each (51.7 s, 1.2 m, 1.0 m). Run 5 output:

```
Running 4 tests using 1 worker
  ✓  1 … Wall chart — flow one, from the login submit › sign in and reach a wall chart in under ten seconds (15.5s)
  ✓  2 … My campaigns › lists my campaigns and opens the wall chart (11.6s)
[hint] anchor clear before the spec scrolled: no
  ✓  3 … Wall chart — first-use rating hint › the rating hint shows once, then stays dismissed (10.9s)
[hint] anchor clear before the spec scrolled: no
  ✓  4 … Wall chart — first-use rating hint › the hinted badge opens its rating control on the first click (10.3s)
  4 passed (1.0m)
```

**For the verifier on the next preview:** the two hint tests must pass and both `[hint]` lines must read `yes` — that is the proof of item 1 (the spec's own scroll only proves the callout). The screenshot script should also load the wall chart unscrolled and confirm the hinted tile is in view and uncovered within ~3 s of the hint appearing, on desktop and at 375 px.
