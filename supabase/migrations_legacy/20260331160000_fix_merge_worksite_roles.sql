-- ============================================================
-- Fix merge_employers: employer_worksite_roles_unique constraint
-- ============================================================
-- The constraint employer_worksite_roles_unique is UNIQUE (employer_id, worksite_id, role_type).
-- The original function attempted to UPDATE employer_worksite_roles SET employer_id = survivor
-- before removing conflicting victim rows, causing a unique-constraint violation when both
-- the survivor and a victim had the same (worksite_id, role_type). The dedup DELETE that
-- followed never ran because the transaction was already aborted.
--
-- Fixes applied:
--   1. Before the UPDATE on employer_worksite_roles, DELETE victim rows that would conflict
--      with an existing survivor row on (worksite_id, role_type).
--   2. The post-UPDATE dedup now partitions on (employer_id, worksite_id, role_type) —
--      matching the actual constraint — and prefers is_current=true rows.
...
BEGIN
  ...
  WHILE v_cur IS NOT NULL LOOP
    IF v_cur = ANY(v_victim_ids) THEN
      RETURN jsonb_build_object(
        ...
        'message', 'One of the employers being merged is an ancestor (parent/grandparent) of the chosen survivor via the parent company field. To resolve: open the survivor employer''s detail card and clear its Parent Company field (or choose a different survivor). The merge can then proceed.'
      );
    END IF;
  ...
  -- FIX: delete victim worksite_roles that would conflict with an existing survivor row
  -- on the unique constraint (employer_id, worksite_id, role_type) before repointing.
  -- The survivor's existing row is kept; victim's duplicate is discarded.
  DELETE FROM employer_worksite_roles ewr
  WHERE ewr.employer_id = ANY(v_victim_ids)
    AND EXISTS (
      SELECT 1 FROM employer_worksite_roles s
      WHERE s.employer_id = v_survivor_id
        AND s.worksite_id = ewr.worksite_id
        AND s.role_type = ewr.role_type
    );
  ...
  WHERE es.id IN (
    SELECT id FROM (
      SELECT id,
             ROW_NUMBER() OVER (
               PARTITION BY employer_id, sector_id
               ORDER BY id
             ) AS rn
  ...
  WHERE et.id IN (
    SELECT id FROM (
      SELECT id,
             ROW_NUMBER() OVER (
               PARTITION BY employer_id, tag_id
               ORDER BY id
             ) AS rn
      FROM employer_tags
      WHERE employer_id = v_survivor_id
  ...
  WHERE pe.id IN (
    SELECT id FROM (
      SELECT id,
             ROW_NUMBER() OVER (
               PARTITION BY employer_id, project_id
               ORDER BY id
             ) AS rn
  ...
  WHERE ws.id IN (
    SELECT id FROM (
      SELECT id,
             ROW_NUMBER() OVER (
               PARTITION BY employer_id, worksite_id
               ORDER BY id
             ) AS rn
  ...
  -- employer_worksite_roles: dedupe on the actual unique constraint (employer_id, worksite_id, role_type).
  -- Prefers is_current=true rows, then shortest notes, then lowest id.
  DELETE FROM employer_worksite_roles ewr
  WHERE ewr.id IN (
    SELECT id FROM (
      SELECT id,
             ROW_NUMBER() OVER (
               PARTITION BY employer_id, worksite_id, role_type
