...
| 1.2 | Navigation driven by modules | PR draft | `feat/oux-wp1.2-nav-modules` | (pending) | 884 tests incl. nine nav-model snapshots, a full-mode literal and a falsifiable reachability suite; full e2e suite green on preview incl. the organiser round trip; screenshots in evidence/wp1.2 | Full-mode sidebar flashes for organisers while the profile loads (WP1.1 fail-open; recorded) | 1 |
