...
## 7. REMAINING work (2 clips + polish)
...
D3 ✅, D4 ✅, E3 ✅ done (§3). Remaining: **B2** (create organising units), **B3** (allocate workers), **A3** (import). Per §7: B2/B3 use the **rule‑based** Campaign Units builder (`create-organising-unit-dialog.tsx`) — Include/Occupation/contains conditions, not a plain dialog; A3 is the **11‑step** worker import (`/workers` → "Import Workers", xlsx `public/excel_worker_import_dummy_1.xlsx`). Reuse the proven pattern: **seed/verify state via Supabase MCP `execute_sql`, thin clip that records the real UI, verify every frame + the DB write.** Then publish the rendered‑but‑unpublished clips (D1/D2/OVERVIEW already in storage; **D3, D4, E3 still need upload** via `TARGET=both node upload-to-storage.mjs` → `node publish-to-app.mjs` → commit) and remind the user to merge `develop → main`. Use the recipe in §6 and verify every frame. Then publish the rendered‑but‑unpublished clips (D1, D2, D3, OVERVIEW → already in storage for D1/D2/OVERVIEW; D3 still needs upload) and regenerate the manifest. Remind the user to merge `develop → main` to push the latest hub to prod.
