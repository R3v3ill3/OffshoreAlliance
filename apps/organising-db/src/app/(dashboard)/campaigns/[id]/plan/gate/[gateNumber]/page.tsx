...
import { GateAssessmentComponent } from '@/components/campaigns/gates/GateAssessment'
import { GateAmbitionAssessment } from '@/components/campaigns/gates/GateAmbitionAssessment'
    ...
    return (
      <div className="p-6 text-center">
        ...
        <Button asChild variant="outline" className="mt-4">
          <Link href={`/campaigns/${campaignId}/plan`}>Back to Campaign Plan</Link>
        </Button>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
        ...
        <ChevronRight className="h-3 w-3" />
        <Link href={`/campaigns/${campaignId}/plan`} className="hover:text-foreground">Plan</Link>
        <ChevronRight className="h-3 w-3" />
      ...
      {useAmbitionMode ? (
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          ...
          <p className="mt-1 text-amber-800">
            Add ambitions on{' '}
            <Link href={`/campaigns/${campaignId}/plan/stage/${gateNumber}`} className="underline font-medium">
              Stage {gateNumber}
            </Link>{' '}
            to assess this gate.
          </p>
      ...
      <div className="mt-6 flex gap-3">
        <Button asChild variant="outline">
          <Link href={`/campaigns/${campaignId}/plan/stage/${gateNumber}`}>
            Back to Stage {gateNumber}
          </Link>
        </Button>
        {gateNumber < 5 && (
          <Button asChild variant="outline">
            <Link href={`/campaigns/${campaignId}/plan/stage/${gateNumber + 1}`}>
