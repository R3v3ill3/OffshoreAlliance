...
| 1.1 | Module registry and workspace mode | PR draft | `feat/oux-wp1.1-workspace-mode` | (pending) | migration `20260909100000_workspace_mode.sql` applied to dev, types regenerated; 780 tests, tsc, build green, lint at baseline; role probe: `user` reads defaults, sees 0 app_settings rows | Two admin-editor advisories deferred; nothing consumes the hook yet (WP1.2/1.4) | 1 |
