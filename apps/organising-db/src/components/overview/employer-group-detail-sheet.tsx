...
import { Building2, MapPin, FileText, Users, Layers } from "lucide-react";

export interface EmployerGroupDetail {
...
interface EmployerGroupDetailSheetProps {
  group: EmployerGroupDetail | null;
  open: boolean;
...
type MemberRow = EmployerGroupDetail["members"][number] & Record<string, unknown>;
type WorksiteRow = EmployerGroupDetail["worksites"][number] & Record<string, unknown>;
type AgreementRow = EmployerGroupDetail["agreements"][number] & Record<string, unknown>;
type WorkerRow = EmployerGroupDetail["workers"][number] & Record<string, unknown>;
type ScopeRow = EmployerGroupDetail["work_scopes"][number] & Record<string, unknown>;

export function EmployerGroupDetailSheet({
  group,
  open,
  onOpenChange,
  campaigns,
}: EmployerGroupDetailSheetProps) {
  ...
  const memberColumns: Column<MemberRow>[] = [
    {
      ...
      render: (row) => (
        <span className="font-medium flex items-center gap-1.5">
          ...
          {row.is_parent && (
            <Badge variant="warning" className="text-xs">Parent</Badge>
          )}
        </span>
      ),
    },
    ...
    },
    {
      ...
      header: "ABN",
      render: (row) => (
        <span className="text-sm font-mono">{(row.abn as string) || "—"}</span>
      ),
    },
    {
      key: "is_active",
      ...
      >
        <SheetHeader className="p-6 pb-4 border-b">
          <div className="pr-6 space-y-1">
            ...
            <SheetDescription>
              {group.is_grouped
                ? `Employer group — ${group.members.length} member${group.members.length !== 1 ? "s" : ""}`
                : "Ungrouped employer"}
            </SheetDescription>
          </div>
          <div className="flex items-center gap-2 pt-1 flex-wrap">
            ...
            <Badge variant="outline">{group.worksites.length} worksites</Badge>
            <Badge variant="outline">{group.agreements.length} agreements</Badge>
            <Badge variant="outline">{group.workers.length} workers</Badge>
          </div>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-6 py-4 border-b">
            <h3 className="text-sm font-semibold flex items-center gap-2 mb-3">
              ...
              {group.is_grouped ? `Group Members (${group.members.length})` : "Employer"}
            </h3>
            <DataTable
              data={group.members as MemberRow[]}
              columns={memberColumns}
              searchPlaceholder="Search employers..."
              searchKeys={["employer_name", "abn"]}
          ...
          <div className="px-6 py-4 border-b">
            <h3 className="text-sm font-semibold flex items-center gap-2 mb-3">
              ...
              Worksites ({group.worksites.length})
            </h3>
            {group.worksites.length === 0 ? (
              <p className="text-sm text-muted-foreground">No worksites linked to this group.</p>
            ) : (
              <DataTable
                data={group.worksites as WorksiteRow[]}
          ...
          <div className="px-6 py-4 border-b">
            <h3 className="text-sm font-semibold flex items-center gap-2 mb-3">
              ...
              Enterprise Agreements ({group.agreements.length})
            </h3>
            {group.agreements.length === 0 ? (
              <p className="text-sm text-muted-foreground">No agreements for this group.</p>
            ) : (
              <DataTable
                data={group.agreements as AgreementRow[]}
          ...
          <div className="px-6 py-4 border-b">
            <h3 className="text-sm font-semibold flex items-center gap-2 mb-3">
              ...
              Workers ({group.workers.length})
            </h3>
            {group.workers.length === 0 ? (
              <p className="text-sm text-muted-foreground">No workers for this group.</p>
            ) : (
              <DataTable
                data={group.workers as WorkerRow[]}
          ...
          <div className="px-6 py-4">
            <h3 className="text-sm font-semibold flex items-center gap-2 mb-3">
              ...
              Work Scopes ({group.work_scopes.length})
            </h3>
            {group.work_scopes.length === 0 ? (
              <p className="text-sm text-muted-foreground">No work scopes for this group.</p>
            ) : (
              <DataTable
                data={group.work_scopes as ScopeRow[]}
