...
| 1.4 | Campaign workspace | PR draft | `feat/oux-wp1.4-campaign-workspace` | (pending) | 1006 tests incl. the 45-surface census and header-action map; full e2e suite green ×1 on the final preview incl. the organiser round trip; screenshots in evidence/wp1.4 | On the default module set only Role check is live in More (others muted, reachable by deep link or Show everything); full-mode users now see the campaign header above the four wizards (intended) | 1, 7 |
