...
| Import the next weekly membership batch as usual, then check: queue rows present, employers/worksites unchanged (Phase 0 exit criterion); work the queue at `/name-reviews` | Phase 0 exit | **Met 2026-09-24 07:04 UTC** (see Phase exits); the operator to confirm who ran the batch, and to work the 20 open queue rows at `/name-reviews` |
