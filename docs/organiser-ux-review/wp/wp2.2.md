...
# e2e against the branch preview — the way to run it (D81): a GitHub-hosted runner via the manually dispatched
# workflow .github/workflows/e2e-preview.yml (Actions tab, or:)
gh workflow run e2e-preview.yml -f base_url=<preview-url>          # -f foreign_campaign_id=3 -f specs="…" optional
# local alternative (credentials from the operator's shell; not from a sandboxed agent runner — D81)
cd apps/organising-db && E2E_BASE_URL=<preview-url> pnpm exec playwright test tests/e2e/structure-api.spec.ts tests/e2e/wall-chart.spec.ts tests/e2e/roles
# sandboxed runner behind a TLS-intercepting proxy CA only (never CI or local; D80): prepend E2E_IGNORE_HTTPS_ERRORS=1
```
...
| **D80** | Playwright: `E2E_IGNORE_HTTPS_ERRORS=1` (opt-in, off by default) sets `ignoreHTTPSErrors` on the config's top-level `use` (both projects and the `request` fixture) and on every hand-made context — global setup's sign-in browser context, `openAdminContext` (`workspace-mode.ts`), the admin context in `wall-chart.spec.ts`, and the `request.newContext()` calls in `campaign-cleanup.ts`, `hint-dismissals.ts` and `structure-api.spec.ts` (none of which inherit `use`). `E2E_IGNORE_HTTPS_ERRORS` is exported from `playwright.config.ts`; `tests/e2e/global-setup.ts` imports it. | The cloud verifier's e2e run failed in global setup with `net::ERR_CERT_AUTHORITY_INVALID` on the first `page.goto` (§9.2): that sandbox's outbound HTTPS is TLS-re-terminated by an agent proxy whose CA is installed for Node (`NODE_EXTRA_CA_CERTS`) and the system store but not for the bundled Chromium (no NSS store, no `certutil`). Never set in CI or locally, so those runs keep full certificate verification; the target is still the branch preview against normal dev. | §5 (e2e command); §9.2 e2e subsection. |
| **D81** | New `.github/workflows/e2e-preview.yml` (`workflow_dispatch`; inputs `base_url` required, `foreign_campaign_id` default `3`, `specs` default = the §4.5 spec list + `wall-chart-decomposition.spec.ts`): one `ubuntu-latest` job, 45-minute timeout, `concurrency` keyed on workflow + ref; a bash guard first (must match `^https://[a-z0-9-]+\.vercel\.app/?$`, must contain `-git-`, must not contain the production ref `gteygwfgjvczanmrwgbr`); checkout / pnpm / Node 22 as `validate-migrations.yml`; `pnpm install --frozen-lockfile`; `playwright install --with-deps chromium`; `pnpm exec playwright test <specs> --reporter=list` in `apps/organising-db` with `E2E_BASE_URL`, `E2E_FOREIGN_CAMPAIGN_ID`, the four `E2E_*` credentials from repository secrets of the same names, `CI=1`, and **no** `E2E_IGNORE_HTTPS_ERRORS`; `test-results` and `playwright-report` uploaded `if: always()` (14 days, `e2e-preview-<run_id>`). `.gitignore` already covers both directories. The operator adds the four secrets (dev accounts only; PROGRESS.md human task). | The sandboxed agent runner cannot host the suite even with D80: through its intercepting proxy the login page takes ~36 s to reach `load` (Playwright's 30 s navigation timeout) and several `_next/static/chunks/*` requests fail with `net::ERR_TOO_MANY_RETRIES`, with HTTP/2 disabled too (§9.2 re-run). The repo had no e2e workflow; a GitHub-hosted runner reaches the preview directly. The workflow only ever targets a branch preview, which talks to normal dev; it never touches production. | §4.5; §5 (e2e command); §9.2 e2e re-run subsection. |
...
#### Stage 3 — contract suite on normal dev, run 2 (after 2.2b) (2026-09-14, child session)

Run on branch `feat/oux-wp2.2-structure-api` at commit `29d655a` (after 2.2b was applied to normal dev), from
`apps/organising-db`, with the command below; exit code `0`; no network retry was needed. Raw output follows (ANSI
colour codes stripped); no diagnosis attempted. The one skipped test is the `OUX_CONTRACT_FOREIGN_USER_*`-gated
permissions test
`a user without write permission on the campaign gets forbidden from every RPC (SKIPPED when OUX_CONTRACT_FOREIGN_USER_EMAIL/_PASSWORD are unset — report the skipped count)`,
which the suite skips because that optional pair was not set, as instructed.
...
OUX_CONTRACT_USER_PASSWORD=<from E2E_USER_PASSWORD> \
OUX_CONTRACT_FOREIGN_CAMPAIGN_ID=3 \
pnpm test:contract 2>&1 | tee /tmp/contract-run-2.log; echo "exit=${PIPESTATUS[0]}"
```
...
The CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.
 ...
 ✓ src/lib/campaign/__contract__/structure-api.contract.test.ts (58 tests | 1 skipped) 98005ms
   ✓ fixture > derived the expected groups and the legacy container has no group (C-e precondition) 479ms
   ✓ structure_group_create > is idempotent for a fixed kind that already exists 476ms
   ✓ structure_group_create > creates a missing fixed kind once and then returns it 1695ms
   ✓ structure_group_create > creates a custom group and rejects a case-insensitive duplicate name (duplicate_group) 977ms
   ✓ structure_group_create > rejects an unknown kind and a blank custom name (22023) 732ms
   ✓ structure_group_update > renames and reorders a custom group 1204ms
   ✓ structure_group_update > renaming a container-backed group renames its legacy container 1068ms
   ✓ structure_group_update > rejects a blank name, a missing group and a group of another campaign (C-i) 1639ms
   ✓ structure_group_reorder > sets display_order by position and keeps unlisted groups after the listed ones 754ms
   ✓ structure_group_reorder > rejects duplicates (22023) 496ms
   ✓ structure_group_delete > empty_only refuses a group that still has units (P0001) 727ms
   ✓ structure_group_delete > cascade_units deletes the group's units and their placements, then the group 1725ms
   ✓ structure_group_delete > rejects an unknown mode (22023) 465ms
   ✓ structure_units_create > creates a container plus members in one call via client_ref and applies assignments with move semantics 2244ms
   ✓ structure_units_create > C-g: a fixed-kind unit cannot be pinned to a custom group; a custom unit can 979ms
   ✓ structure_units_create > is atomic: a valid first element and an invalid second leave nothing behind 933ms
   ✓ structure_units_create > C-i: a parent from another campaign is rejected (22023); an unknown parent is not_found 699ms
   ✓ structure_units_create > rejects an unknown ou_type and a non-member assignment (22023) 693ms
   ✓ structure_unit_update > applies a whitelisted patch (estimated_size alias) and returns the keys 1209ms
   ✓ structure_unit_update > rejects keys outside the whitelist (description, leader_worker_id, ou_type) with 22023 1655ms
   ✓ structure_unit_update > not_found for a missing unit and 22023 for another campaign's unit (C-i) 1499ms
   ✓ structure_unit_reorder > sets a 0-based display_order for the listed units 1196ms
   ✓ structure_unit_reorder > rejects duplicates and foreign units 703ms
   ✓ structure_unit_delete > moves and removes placements per reassignment, carries the primary (C-h), and removes the rest 3808ms
   ✓ structure_unit_delete > detaches children by default and deletes them with deleteChildren 2055ms
   ✓ structure_unit_delete > is atomic: a bad second reassignment leaves the unit and its placements untouched 1500ms
   ✓ structure_unit_merge > re-points, collapses (primary OR-ed), preserves provenance (C-l), re-points unit rules and deletes the sources 4032ms
   ✓ structure_unit_merge > C-j: refuses a cross-group merge and a survivor that is also a source; nothing changes 2388ms
   ✓ structure_unit_merge > legacy check_worker_ou_group_exclusivity still refuses merging worksites of two different Employer containers (P0001 → rule_violation; wp2.2.md §3.4 C-j note) 2226ms
   ✓ structure_unit_merge > refuses a source that has child units (P0001) 1191ms
   ✓ structure_unit_split > C-k: same-group children take the workers out of the source; keepInSource is ignored and reported as displaced 3309ms
   ✓ structure_unit_split > cross-group children honour keepInSource (kept) or move the source row (moved) 2932ms
   ✓ structure_unit_split > is atomic: assigning one worker to two siblings raises duplicate_in_group and creates no children 1654ms
   ✓ structure_unit_split > rejects a fixed-kind p_group_id (C-g) 485ms
   ✓ structure_units_bulk_save > deletes, updates and creates in one call 1898ms
   ✓ structure_units_bulk_save > is atomic: a unit both deleted and updated is refused and nothing changes 1494ms
   ✓ structure_placements_assign > inserts, skips a same-group conflict by default, moves with onConflict move, errors with onConflict error (C-a) 2393ms
   ✓ structure_placements_assign > C-e: a legacy container without a group rejects placements (P0001); an Employer container with a group accepts them 1190ms
   ✓ structure_placements_assign > C-h: isPrimary makes the placement the single campaign-wide primary 1711ms
   ✓ structure_placements_assign > rejects a non-member (22023), an unknown worker (P0002), a bad source and a bad conflict mode (22023) 1661ms
   ✓ structure_placements_move > C-b/C-l: a move re-points the source row (same id, same provenance) and displaces the target group's other placement 2115ms
   ✓ structure_placements_move > re-issuing a completed move is a no-op (skipped), and a missing source row is not_found 1221ms
   ✓ structure_placements_move > dragging from Unassigned inserts a manual row and displaces the worker's other placement in that group 1226ms
   ✓ structure_placements_move > C-c / K1: copy is allowed across groups and refused within a group (23505, state unchanged) 1904ms
   ✓ structure_placements_move > C-d: unassign within one group removes only that group's placement; without a group removes all 2375ms
   ✓ structure_placements_move > keepInParent creates the Employer-container placement for a worksite child target (M2 going forward) and can be turned off 2758ms
   ✓ structure_placements_move > legacy check_worker_ou_group_exclusivity still refuses a move between worksites of two different Employer containers (P0001 → rule_violation; wp2.2.md §3.4 C-b note) 1712ms
   ✓ structure_placements_move > rejects same source and target, withinGroupId with a target, and a container without a group (C-e) 969ms
   ✓ structure_placements_unassign > removes one placement, a group's placement, or everything; idempotent 2192ms
   ✓ structure_placements_set_primary > C-h: clears the other primary and sets this one; not_found without a placement 2146ms
   ✓ structure_placements_replace_rule_rows > R1: replaces rule rows only; manual and universe rows survive; same-group manual rows win (skipped) 2129ms
   ✓ structure_placements_replace_rule_rows > rejects a container target, a foreign rule id and a malformed element (22023) 1008ms
   ✓ structure_materialise_employer_placements > M2: gives every worksite-child member one universe placement on the Employer container; idempotent 2553ms
   ✓ permissions (42501 from the pre-check, never a silent no-op) > the main user gets forbidden on OUX_CONTRACT_FOREIGN_CAMPAIGN_ID (always runs) 2080ms
   ✓ permissions (42501 from the pre-check, never a silent no-op) > a missing campaign is not_found 483ms
   ✓ invariants after the suite > C-a: no (worker, group) pair holds two placements; C-h: no worker holds two primaries 1204ms
   ✓ invariants after the suite > C-f: no RPC created a unit named Unassigned 467ms

 Test Files  1 passed (1)
      Tests  57 passed | 1 skipped (58)
   Start at  22:15:44
   Duration  102.15s (transform 420ms, setup 0ms, collect 1.31s, tests 98.00s, environment 0ms, prepare 145ms)
...
> **Re-run note (D80):** the re-run in the same sandbox uses `E2E_IGNORE_HTTPS_ERRORS=1`, an opt-in that makes
> Chromium and the `request` fixture accept the sandbox proxy's TLS-re-terminating CA (installed for Node and the
> system store, not for the bundled Chromium under `/opt/pw-browsers`); never set in CI or locally. Nothing about the
> product or the preview was wrong.
...
> **D81:** this sandbox cannot host the browser suite (see the timings and `net::ERR_TOO_MANY_RETRIES` below); the e2e
> is run on a GitHub-hosted runner through the manually dispatched `.github/workflows/e2e-preview.yml`.
