...
| D40 | **Hotfix 2 — the parent's name never arrived; the loader no longer embeds.** Production after hotfix #56 (`campaigns!parent_campaign_id(campaign_id, name)`, D39): shared assessments, the Colour-by group and cross-campaign rating all worked, but the header showed "Part of campaign 64" (the id fallback) and the Assessments tab "Shared from parent campaign" (`familyLabel(null)`). Diagnosis: on a self-referencing FK PostgREST resolves the column hint in the one-to-many direction — the campaign's CHILDREN, an array — not many-to-one (the parent), so the embed was an empty array or a child whose id ≠ `parent_campaign_id`, and the parser yielded `parentName: null`. The direction cannot be proven against dev (anon has no row access), so the ambiguity is designed out: `loadCampaignParent` makes two plain primary-key reads — `select("parent_campaign_id").eq("campaign_id", id).maybeSingle()`, then, when set, `select("campaign_id, name").eq("campaign_id", parentId).maybeSingle()` — returning `{ parentId, parentName }` (name null when the second read returns no row or a blank name; either error thrown with the `campaign-parent:` prefix). No select string names a relationship (the unit test asserts no `!`, no `(`). `CAMPAIGN_PARENT_SELECT` is now the first read's column list, `CAMPAIGN_PARENT_NAME_SELECT` the second's; `parseCampaignParent` and the embed types are deleted (only the test imported them). Harness (additive): the fake backend's `.eq("campaign_id", v)` on the `campaigns` table now narrows the rows (string-compared) because a family fixture holds several campaign rows and read 2 must return the parent's; every other filter is still ignored; the `family` knob appends the parent's own `campaigns` row (`campaign_id: parentId, name`) and no longer carries an embedded `parent` field. Cost: one extra PK round trip per parent load, cached five minutes on the client. | A PostgREST string proven only against the fake was wrong twice (D39, D40); the standing rule from hotfix #56 applies, and this design has no string to prove. | §3.5 |
| D41 | **Follow-up (operator, 2026-09-22): "Part of" on the Settings page too, by extraction.** The Part-of control and its reads left `campaign-basics-edit-sheet.tsx` for a shared component `components/campaigns/campaign-parent-select.tsx` — `CampaignParentSelect({ campaignId, value, onChange, disabled?, enabled?, id? })`, `useCampaignParentCandidates` (the F7-filtered candidates, server and client predicates), `useCampaignChildren` (`["campaign-children", "<id>"]`, the "has children" query both callers share), `parentSelectHint` — and the save rules for a pure module `lib/campaign/campaign-parent-form.ts`: `currentParentIdOf`, `resolveParentChange` / `applyParentChange` (the column joins the payload only when it changed, F1/D32), `campaignSaveError` (`campaign_family_*` → sentence), `afterCampaignParentSaved` (family invalidations + `campaign_parent_set`, only on a change). The sheet keeps its behaviour byte for byte (its jsdom suite is unchanged and green; the D22 current-parent rule keys on the select's value as before, so the `currentParentId` prop the brief sketched was not needed). `campaign-settings.tsx` (§3.15 "not touched" — opened under this operator request; **exact edited ranges, new-file numbering, for the WP2.7 planner: `:44–51` (two imports), `:144–145` (`parent_campaign_id: ""` in the `basics` state), `:357–358` (hydration from `campaign.parent_campaign_id`), `:422–445` (in `saveBasicsMutation`: `applyParentChange` before the update, `campaignSaveError` on error, `return { nextParentId }`, `afterCampaignParentSaved` in `onSuccess`), `:872–879` (the `<CampaignParentSelect id="settings-parent" …/>` block directly under the organiser field's grid, above "Start date")** — nothing else in the file, not the units section nor the allocation save). The Settings page has no jsdom harness, so the shared component has its own small jsdom test (renders the writable candidates only and hands back the id; keeps a non-writable current parent; disabled with the reason when children exist) and the pure module a node test (7 cases). The Settings toast shows the mapped sentence through the existing `onError` (`error.message`). | The operator could not find "Part of" on the Settings page; the header sheet and the Settings section must show the same information and control, and one implementation keeps them identical. | §3.7 "Basics sheet", §3.15 |
