...
7. **Snapshot tiles follow the owner filter for Live / Drafts & paused / Finished, but not for Awaiting review** (`ActionsHubPage.tsx:164-193`). §2.10 requires the first three to count all six kinds, which they do, and they are counted from the rows after the owner filter, so the tiles and the table describe the same set. "Awaiting review" is the exception: moderation is a duty over every relay in the org, whoever set it up, so its number must not change when the owner filter does. Because the rows themselves are narrowed server-side by `owner=mine_or_unowned` (fix round 1, finding 4), summing them could not produce that number — it would have said "awaiting *my* review" under Mine. So `/api/sms/activity` returns `pending_moderation_total`, computed server-side over every **non-archived** relay the caller can read under RLS, with **no owner predicate and no LIMIT**, whatever the owner filter asked for; the pure `pendingModerationTotal(response)` reads that field and the tile renders it. The per-row "N to review" badges are unchanged — those describe one relay each and correctly follow the rows. Pinned by "does not move when the owner filter does" and "ignores the rows entirely" in `hub-rows.test.ts`.
...
| 3 | The Archived chip printed the server's org-wide `archived_total` while every other chip was filtered | Archived rows are only fetched once the toggle is on, so until then the count is unknowable: `counts` carries `archivedUnknown` and `ActionsTable` renders "…" on that chip. `ChipRow`'s `count` accepts `number \| string`. The `Show archived (N)` toggle keeps showing the server's `archived_total`, which — because finding 4 gave that count the same owner predicate as the list — is the **mine-or-ownerless** archived total under Mine and the org-wide total under All: how many rows the toggle would fetch, which is what that control is about. The chip shows the filtered count of what was fetched. _(Wording corrected in round 2: the round-1 note called it "the server total" without saying that the server total is owner-scoped.)_ |
...
### Fix round 2 (reviewer findings, 2026-09-09)

Commit `debea32` — `fix(oux-wp1.5): reviewer round 2 — moderation is org-wide, honest params`.

| # | Finding | What changed |
|---|---|---|
| A (blocking) | The "Awaiting review" tile summed `pendingModerationTotal(allRows)`, and round 1's finding 4 had made those rows server-filtered by the owner param — so under the default Mine view the tile counted only the caller's own relays, contradicting its own comment and deviation 7 | Fixed **without** re-widening the row fetch. `/api/sms/activity` now returns `pending_moderation_total`, computed by a new `countPendingModeration()` over every **non-archived** relay in scope with **no owner predicate and no LIMIT** — still under the caller's RLS, so it is only ever the total they may see. The archived exclusion that round 1's finding 7 put in the client helper moves server-side with it (`.is('archived_at', null)` on the relays). `pendingModerationTotal` becomes a pure selector over the response, `useHubActionRows` exposes it, and the tile renders it. The per-row "N to review" badges are untouched. New tests: "does not move when the owner filter does" (the mine-narrowed and the everyone payload for one org carry different relay rows and the same total), "ignores the rows entirely", "is zero before the route has answered". Deviation 7 rewritten to say exactly this. |
| 2 | Flipping Mine↔All emptied the table and the tiles for a beat, because the two are separate cache entries | `placeholderData: keepPreviousData` (TanStack Query v5.95.2, the installed version; `useEmailInbox.ts:115` already used the equivalent `(previous) => previous`) on all three activity queries — `useSmsActivity`, `useEmailActivity`, `useCallActivity`. The previous answer stays on screen until the new one lands. |
| 3 | §6 fix round 1, item 3 described the `Show archived (N)` toggle as showing "the server total" without saying that finding 4 had given that count the same owner predicate as the list | Wording corrected in the round-1 table: the toggle shows the **mine-or-ownerless** archived total under Mine (the org-wide one under All) — how many rows it would fetch — and the chip shows the filtered count. No code change; the behaviour was already what the toggle is for. |
| 4 | The 200-row cap disclosure was inside `rows.length > 0`, so it vanished exactly when a filtered view came back empty over a truncated read — the one time it changes what the organiser should conclude | `useHubActionRows` returns `capped`, true when any single source read came back at the cap (blasts + chats are counted together: one `sms_lists` query), and `ActionsTable` renders the line on `capped` alone. The cap is now one exported constant, `HUB_SOURCE_LIMIT`, used by the three routes, the hook and the sentence, so they cannot disagree about where the list stops. |
| 5 | `?mine=1` was not honest: the predicate is `created_by = auth.uid() OR created_by IS NULL` | Renamed to `?owner=mine_or_unowned` on all three activity routes and in both hooks, with the route comments rewritten to say why. No spec asserted the API parameter. The **page's** `?mine=0` is a different thing — the URL-held owner filter — and is unchanged, so every hub link still works. |
| 7 | `useHubActionRows` exposed a dead `isError` | Removed from the interface and the return. Every consumer already reads the three per-source flags, which is what the inline retry strips are built on. |
| 1 | The hook's header said the three-query design was kept because `useSmsActivity` is "shared with the campaign SMS panels" | It is not: `useSmsActivity` has exactly one consumer, this hook. Comment corrected — the design is kept because the query key is the `['sms-activity']` prefix every SMS mutation invalidates (`useSmsActionOps.ts:89`, `SmsCreateActionPage.tsx:281,310`), because three queries fail independently, and because it already polls while a send is in flight. |

Gates, from `apps/organising-db`:

- `pnpm test` — **61 files, 828 tests passed** (827 + 3 new moderation tests − 2 replaced; no failures, no skips).
- `pnpm exec tsc --noEmit -p tsconfig.json` — clean, exit 0.
- `pnpm exec eslint <9 touched files>` — zero findings.
- `pnpm exec eslint` (whole app) — **294 problems (143 errors, 151 warnings)**, identical to the recorded baseline.
- `pnpm build` — exit 0; `/actions`, `/api/email/activity`, `/api/calls/activity` all present.
- `env -u E2E_USER_EMAIL -u E2E_USER_PASSWORD pnpm e2e` — **4 skipped, exit 0**.

Not run: a credentialled `pnpm e2e` against a branch preview — still no preview URL or `E2E_USER_*` in this environment.
