...
The counts and name lists in this document come from read-only aggregate queries run against the production Supabase project (`gteygwfgjvczanmrwgbr`) on 22 September 2026 through the Supabase MCP connector, plus one query against the realistic clone (`yqjkuobcawvigsfpgrcm`) for campaign definitions. No personal field was selected: the queries return counts, distributions, employer names, worksite names, agreement names and occupation titles only, and nothing was written. At the time of the first draft this contravened the programme's standing note that production is never read by an agent (`docs/organiser-ux-review/PROGRESS.md`, "Supabase projects"). **Decision D0 (operator, 22 September 2026): agents may run read-only profiling on production for this workstream.** The standing note now carries a dated amendment to that effect. The profiling pack under `scripts/data-hygiene/oa-universe/` is the sanctioned instrument; any query outside it stays selects-only and PII-free by the same rule, and every write to production remains operator-run under the promotion gate.

### Phases at a glance

| Phase | Weeks | What it does | What you get |
|---|---|---|---|
| 0 Stop the bleed | 1–2 | Remove the synthetic dataset; imports stop creating employers and worksites (alias lookup, one matcher, a review queue with search-and-map); fix the alias constraint; capture the vessel-tracking tables as a migration; baseline profile | Density figures without 665 phantom members; no new duplicates from the weekly membership files |
| 1 Canonical registers | 2–4 | Adjudicate every employer and worksite: legal-entity rule, merges with aliases, subsidiaries restored, kinds set, fleets and hubs built, the 24 missing assets created, one scope taxonomy | One row per real organisation and place, every variant an alias; every facility and contractor on the map resolvable |
| 2 Structure | 4–6 | Provenance columns; engagements (evolved from `worksite_contracts`); worksite relationships; agreement holders and coverage; compatibility views | The grain the map is built on, with confidence, source and validity on every fact; nothing existing breaks |
| 3 Load the map | 6–7 | Facility × scope incumbents, operator–company pairs, vessels and crew providers, engagements derived from worker records; a conflicts list for the owner | The sector map in the database and queryable, research and organiser knowledge side by side |
| 4 Members and imports | 7–9 | Re-point workers to engagements; placement history; import hardening; placeholder retirement | Every member attached to who they work for, where, on what scope; imports that cannot regress it |
| 5 Hierarchy, patches, reports | 9–11 | Engagement-keyed units; patch model and workload views; density, coverage, footprint and workload reports; campaign universes built from engagements | Reports, work allocation and campaign set-up from one structure |
| 6 Authoritative coverage, steady state | 8 onward | FWC coverage clauses read into agreement coverage; quarterly re-verification; review queue worked weekly with the membership batch | Coverage that stands up; data that stays clean |

Operator time concentrates in Phase 1 (adjudication sessions) and at each production run under the promotion gate. Phase 5 is sequenced with the organiser UX programme's WP2.7 and WP3.9. Details per work package are in §5.
...
| T. Same-day batches as the `TestCo` rows (synthetic, confirmed) | 9 and 16 Apr 2026 | 8 | not in the OA Universe; every worker on a `TEST` worksite; no membership numbers; no agreements | `TestCo Energy`, `TestCo 2`, `Fortis Maintenance Services`, `Aegis Offshore Maintenance Pty Ltd`, … |
...
- **Merges and their aftermath.** All 23 merges ran on 31 March 2026, before any worker row existed, so no merge has ever moved a worker (`employer_merge_events.payload.workers_updated` is 0 on every row). One alias is wrong: `MMA` → Monadelphous (MMA is MMA Offshore, now Cyan Renewables, which exists separately as 705 with 49 workers and 16 vessel roles; the two MMA vessel-operations agreements sit on a re-created `MMA` row, 699). Two merges were right — the operator confirms IAS, Innovative Asset Solutions, was acquired by UGL in 2021 and Rigforce by Programmed — yet `IAS Group` (827, 5 workers) was created by the September membership sync and `Rigforce Pty Ltd` (798, 15 workers, 5 vessel roles) by a June worker import, because neither import reads the alias table. The aliases were correct and useless.
...
### 1.11 Tables in production that are not in the repository

Production holds 264 tables in `public`. Sixteen of them appear in no file under `supabase/migrations/`, `supabase/migrations_legacy/` or `supabase/reconciliation/`, and nothing under `apps/` or `packages/` reads them: `vessels`, `geofences` and fourteen `mobilisation_*` tables (`sources`, `signals`, `rules`, `alerts`, `alert_signals`, `alert_events`, `notifications`, `positions`, `prefs`, `push_subscriptions`, `settings`, `watch_contractors`, `watch_keywords`, `watch_vessels`). They are the vessel-tracking module the context note mentions — a separate development piece, the operator confirms, that watches for purpose-specific vessels moving towards the relevant fields, because construction and decommissioning projects have a long lead time between mobilisation and arrival in which organising intervention is possible — and they are live: `vessels` holds 25 rows with IMO, MMSI, name, type, owner name, `owner_operator_id` (a foreign key to `employers`), relevance, last AIS position and geofence membership; `mobilisation_watch_contractors` holds 14 rows with a `canonical_name`, an `aliases` array, an `employer_id` and a `tier`; 19 signals and 18 alerts have been recorded, several keyed to `contractor_id`, `operator_id` and `worksite_id`.

Three consequences for this plan:

- **Vessel identity already has a home.** §3.2 no longer proposes IMO, owner and position columns on `worksites`; a vessel-grain worksite links to `vessels.vessel_id`, and presence (current / recent / likely / historical) sits beside the AIS position that table already tracks.
- **A fourth alias store.** `mobilisation_watch_contractors.aliases` is a per-contractor list of name variants used for signal matching. It must read from, or be folded into, `employer_name_aliases`, or the tracker and the imports will disagree about who a contractor is.
- **Ledger governance.** The programme's rule is that every schema change is a migration file merged before it is applied. These tables bypass that, so the repository cannot rebuild production, the generated types do not know them, and the clone will not have them. D15 asks how they came to be applied and asks for a migration that captures them (`supabase db diff` against production is the mechanical route) before Phase 2 adds anything that references `vessels`.
...
**Rule (Q-S1, Q-S2; operator, 22 Sep 2026): an employer row is a legal entity.** A group root (Monadelphous Group, Programmed / PERSOL, Chevron Australia) has its agreement-holding subsidiaries as children through `parent_employer_id` (MEA, M&ISS, M Maintenance Services, MWOG; Programmed Marine, RFM Offshore, Rigforce), each flagged `is_agreement_entity`. A lineage-A workgroup row that is *not* a legal entity (`CHEVRON GORGON OPERATIONS`, `SGS PRELUDE CHEMISTS`, `WOODSIDE ENERGY LTD NGUJIMA-YIN AND OHKA FPSO`) is merged into the legal employer, its name kept as an alias, and the facility and scope it carried move onto the agreement: holder = Chevron Australia, coverage = Gorgon (an `agreement_worksites` row), scope = production operations (an `agreement_scopes` row). Where the workgroup name embeds both the entity and the scope (`CHC HELICOPTER (AUSTRALIA) AIRCRAFT ENGINEERS`) the entity is CHC Helicopter (Australia) and "aircraft engineers" becomes the agreement's occupational scope. `merge_employers` already re-points workers, agreements, roles, junctions and campaign universes, so the mechanics exist; what is new is writing the coverage rows in the same script.

Add: `organisation_kind` (`operator_tier1`, `operator_tier2`, `facility_operator`, `prime_contractor`, `contractor`, `crew_provider`, `labour_hire`, `union_staff`, `out_of_universe`, `placeholder`), `is_agreement_entity`, `legal_name`, `abn` populated, `confidence`, `source`, `verified_at`, `verified_by`, `notes`. Keep `parent_employer_id` (add a cycle guard) and retire the free-text `parent_company`. Extend `employer_name_aliases.source` to `merge | manual | import | oa_universe | fwc` and add a unique index on the normalised alias so a variant, once recorded, can never spawn a second row; fold `mobilisation_watch_contractors.aliases` into the same table (§1.11). Retire `employer_category` in favour of `organisation_kind` once reports are re-pointed (or map the six existing values onto it). Out-of-universe employers are kept with `organisation_kind = out_of_universe`, hidden from organising views, and the determination is an admin's, row by row (D11).

### 3.2 Worksites

Add: `grain` (`basin_region`, `hub_field`, `facility`, `onshore_plant`, `vessel`, `fleet`, `project`, `coverage_area`, `office`, `out_of_universe`), `facility_operator_id` beside `principal_employer_id` (rename the latter's meaning to *titleholder* in documentation and UI), `status` (`planned`, `construction`, `commissioning`, `operating`, `shut_in`, `ceased`, `decommissioning`, `removed`, `historical`) with `status_as_at`, `is_normally_unmanned`, `sub_sector`, `jurisdiction`, `region`, `vessel_id` (a foreign key to the existing `vessels` table, §1.11, which already carries IMO, MMSI, owner and position), `presence` (`current`, `recent`, `likely`, `historical`) with `presence_as_at`, and the provenance columns. **Fleets (D9):** a fleet is a worksite of grain `fleet` whose `principal_employer_id` is the vessel operator; every named vessel of that operator has `parent_worksite_id` = the fleet, so the wall chart and reports can treat "all of DOF's vessels" as one node, and the fleet row itself is the placement target for a worker whose vessel is not yet known — the generic placeholder the operator asked for, without a separate placeholder row. Add `worksite_relationships (from_worksite_id, to_worksite_id, relationship, valid_from, valid_to, source, confidence)` with relationships `part_of_hub`, `feeds`, `paired_onshore`, `successor_of`; keep `parent_worksite_id` as the primary hub and fleet link and add the cycle guard it lacks. Add `worksite_merge_events` mirroring the employer one, and a worksite merge routine that re-points workers, roles, agreement links, campaign universes, unit bases, programs and projects. Coverage-area rows stay for agreements and are refused as worker placement targets by a check (Q-S4); placeholder rows are retired in favour of a nullable worksite plus a `placement_status` reason (Q-S3).
...
The word *scope* does three jobs in the database and the map, which is why Q-S7 was hard to answer:

| Meaning | Where it lives today | Where it lives in the target model |
|---|---|---|
| The kind of work a contractor performs at a facility (catering on Goodwyn A; HUC on Scarborough) | `work_scopes` (22 leaves under Brownfields / Service / Specialist / Marine), `worksite_scopes`, `employer_scopes`; the map's 14 scope columns; `upcoming_project_employers.role_type` (14 values) | one `work_scopes` tree; the scope on an engagement |
| What an agreement covers: which facilities, which work, which occupations | `agreements.agreement_scope` (site / project / sector / company breadth, null everywhere), `agreement_worksites`, the empty `agreement_scopes`, and the facility or occupation baked into the holder's name (`… PRELUDE CHEMISTS`, `… AIRCRAFT ENGINEERS`) | `agreement_worksites` (facilities) + `agreement_scopes` (work scopes and occupation groups) + `agreement_scope` (breadth) |
| The bargaining stream an agreement belongs to (Catering, Drilling, Marine – Deck Officers) | `sectors` (16 rows, the tabs of the agreements spreadsheet) and `agreements.source_sheet` | `sectors`, unchanged, on the agreement |

The "four vocabularies" of Q-S7 are the four sources in the first row. **Decision D7:** adopt the map's 14 scopes as the top level of `work_scopes` (Catering/FM, Maintenance/brownfield, HUC/commissioning, Inspection/integrity, Cranes/lifting, Helicopters, Marine supply/vessels, Crewing/labour hire, ROV/subsea/IMR/diving, Drilling, Construction/EPC/pipelay, Decommissioning, Survey/positioning, Other), keep the existing leaves (Electrical, Mechanical, PFP, …) under them, and add `scope_crosswalk` rows mapping `sectors` (bargaining streams) and `upcoming_project_employers.role_type` to scopes. `sectors` stays as the bargaining-stream dimension on agreements.
...
- **Patch (D10)**: a patch is a named, dated set of assignments over *any* of the vectors, mixed freely — facilities, employers, scopes, regions or basins, engagements, campaigns and campaign families — so it covers campaign and non-campaign territory alike: an assignment to a facility or an employer resolves through engagements to workers whether or not a campaign exists there. `organiser_patches` gains `valid_from` / `valid_to`; `organiser_patch_assignments.entity_type` gains `engagement`, `campaign`, `scope`, `region`, `worksite_group`, with a uniqueness constraint on (patch, type, id) and a trigger that checks `entity_id` exists in the table the type names. `v_patch_membership` resolves assignments → engagements → workers; `v_patch_workload` rolls up members, density, agreements expiring and open campaign tasks per organiser; `v_patch_overlaps` lists workers reachable from more than one patch. Overlaps are allowed and reported, not forbidden. The *allocation logic* — which vectors an organiser's patch should be built from, and whether one patch may combine them — is still being determined; DA5.2 starts with these views over the two existing patches and the organiser-to-campaign assignments, so the rule is chosen against real membership counts rather than in the abstract.
...
| # | Question | Recommendation | Decision (operator, 22 Sep 2026) |
|---|---|---|---|
| Q-S1 | Are lineage-A rows (`CHEVRON GORGON OPERATIONS`, `SGS PRELUDE CHEMISTS`, `CHC HELICOPTER (AUSTRALIA) AIRCRAFT ENGINEERS`) employers or agreement workgroups? | Keep as `is_agreement_entity` children of the legal employer; move the facility into an engagement; never delete. | **Agreement scopes, not employers.** Chevron is the employer, Gorgon the worksite and the agreement's scope; SGS is the employer, Prelude the worksite and "Prelude chemists" the scope; CHC Helicopter (Australia) is both the employer and the agreement, "aircraft engineers" its scope. Recorded as the legal-entity rule in §3.1: merge into the legal employer, coverage and scope onto the agreement. |
| Q-S2 | Should agreement-holding subsidiaries collapsed by merges (Monadelphous ×4, Programmed ×4) be restored? | Yes, as agreement entities under the group root. | Agreed. |
| Q-S3 | Placeholder policy: `Unknown`, `Unemployed`, `Not Currently Deployed`, `Unspecified`, `CASUAL EMPLOYEES`. | Allow null employer/worksite with a `placement_status` reason code; retire the placeholder rows. | Agreed. |
| Q-S4 | Coverage-area worksites (`WA/NT Offshore (General)`). | Keep as grain `coverage_area` for agreements; block them as worker placement targets. | Agreed. |
| Q-S5 | Fleet placeholders (`DOF Vessels`, `Skandi Vessels`, `Go Offshore`). | Keep as grain `fleet`; workers may sit there until the vessel is known. | Agreed, and extended: a fleet is a parent node holding the operator's vessels and doubling as the vessel-unknown placeholder (§3.2, D9). |
| Q-S6 | Facility operator vs titleholder on Pyrenees, Barossa, Ningaloo Vision, historical MODEC vessels. | Two roles with validity dates; contractor engagements attach to whichever principal the contract names. | Agreed. |
| Q-S7 | Which of the four scope vocabularies is canonical? | The map's 14 scopes on top of the existing `work_scopes` leaves; `sectors` stays as the bargaining stream. | Agreed on the §3.5 model. The operator noted that "scope" names contractor work, agreement coverage and sub-sector alike; §3.5 now separates the three meanings. |
| Q-S8 | Is Vermilion / Wandoo in the universe? Qube? Alkimos? | Owner call. | Wandoo and Qube in, Alkimos out. |
| Q-S9 | How much history: from 2024 (map's window) or from the earliest agreement? | Effective-date from the earliest fact we hold; do not invent history. | Agreed: do not invent history and do not spend effort researching facts we do not hold. |
| Q-S10 | Should the membership sync create employers/worksites for unmatched names? | No: queue them (`needs_review`) with the top-3 proposals; an admin confirms or creates. | Agreed: no auto-create; queue with the top-3 proposals, and let the reviewer search and map to an existing row before creating. |
...
| DA0.2 | Remove the synthetic dataset (D4, confirmed): employers 787, 791 and the six same-day contractors 788–790, 792–794; worksites 196–199; campaigns 15 and 37; program 6; projects 18–21; 11 employer–worksite roles; 664 workers (none with a membership number) and their cascaded rows. Order: campaigns → workers → projects → program → worksites → employers, because `workers.employer_id`, `workers.worksite_id` and `projects.worksite_id` are `NO ACTION` while the campaign and role junctions cascade. The preflight lists child-row counts per table (SMS, email, call and rating rows attached to the 665) and worker 1536, the operator's own test record, which is kept: re-pointed to AWU WA Branch / AWU Head Office before the TestCo rows go and removed from campaign 64 (D16). Rehearse on the clone with a rollback; then the operator runs it on production | `00_preflight.sql`, `10_remove_test_dataset.sql`, `90_rollback.sql` | active workers fall by 664 to 5,085; `05_candidate_clusters.sql` no longer lists a `test` cluster; campaigns 15 and 37 gone |
...
| DA0.4 | The workstream's ledger row, decision register and worksheet conventions; D0 recorded as an amendment to the programme's standing note | this document's §6, `PROGRESS.md` standing note | operator sign-off |
...
| DA1.2 | Worksites round: adjudicate the 194-row worksheet; merge duplicates with a new `worksite_merge_events` routine; set grain, offshore flag, status, titleholder and facility operator; create the 24 missing assets from the Key Assets tab with `source = oa_universe_research`; hub links for NWS, Ichthys, Varanus, Barossa; fleet parents for each vessel operator with their vessels beneath (D9); vessel-grain rows linked to `vessels.vessel_id`, with the key-clients tab's 38 vessels reconciled against the 25 rows that table already holds (D15) | decisions Q-W1…Q-W11; scripts `10_worksites_*.sql` | every facility × scope row maps to exactly one worksite; 0 worksites of type `Other` without a grain |
...
| # | Decision | Recommendation | Blocks | Status (operator, 22 Sep 2026 unless stated) |
|---|---|---|---|---|
| D0 | May agents run read-only profiling on production for this workstream? | Operator runs the pack; agents use the clone. | DA0.1 | **Decided:** agents may run read-only profiling on production. Standing note amended (DA0.4); writes remain operator-run. |
| D1 | Extend existing tables (§3) rather than introduce a parallel "organisations / facilities" model | Extend | Phase 2 | **Decided:** extend, unless a compelling reason for a parallel model emerges. None identified; the test is a consumer of `employer_id` / `worksite_id` that cannot tolerate the grain change. |
| D2 | Q-S1: lineage-A rows | Keep as agreement-entity children | DA1.1 | **Decided, amended.** The operator's reading is confirmed: those rows were created around agreement scope, so one employer with several agreements became several employer rows. They are merged into the single legal employer and the facility and scope move onto the agreement (§3.1). Subsidiaries that are legal entities stay as children (D3). |
| D3 | Q-S2: restore collapsed agreement-holding subsidiaries | Accept for Monadelphous and Programmed; others case by case | DA1.1 | **Decided:** as recommended. Note 2026-09-24 (S10–S14): business units that hold their own agreements are children of the group root so a campaign can key on the unit without the rest of the group (Programmed, Downer, Monadelphous); Isologics (88) is a child of KAEFER (704) on the operator's knowledge. |
| D4 | Q-E19: the April synthetic dataset is removed from production | Confirm identity first; then remove with rollback | DA0.2 | **Decided:** remove the TestCo-related data. Identity confirmed the same day: every worker of the six same-day contractors sits on a `TEST` worksite and none of the 665 has a membership number. |
| D5 | Q-S10: imports stop auto-creating employers and worksites; unmatched names queue | Accept | DA0.3 | **Decided:** accept, with search-and-map to an existing row before create. |
| D6 | `engagements` evolves `worksite_contracts` (rename + columns) rather than a new table | Evolve | DA2.2 | **Decided:** evolve. |
| D7 | Q-S7: scope taxonomy | Map's 14 scopes on top; sectors stay as bargaining stream | DA1.3 | **Decided:** the §3.5 model. **Amended 2026-09-24 (DA1.3 Q1):** a fifteenth top-level scope, `production_operations`, for the operators' own production workforce (19 agreements in the Production stream). |
| D8 | Q-S6: two operator roles per worksite with dates | Accept | DA1.2 | **Decided:** accept. |
| D9 | Q-S3 / Q-S4 / Q-S5: placeholders, coverage areas, fleets | Retire placeholders; keep coverage areas and fleets as grains | DA1.2, DA4.3 | **Decided, amended:** as recommended, plus fleets as parent nodes holding their vessels and serving as the vessel-unknown placeholder (§3.2). |
| D10 | Patch model (§3.6) with overlaps allowed | Accept | DA5.2 | **Decided in principle:** patches cover campaign and non-campaign areas over any mix of vectors. Allocation logic is still being determined and is deferred to DA5.2 discovery. |
| D11 | Q-S8: universe boundary | Owner rules per row; `out_of_universe` kept, hidden | DA1.1, DA1.2 | **Decided:** Wandoo and Qube in, Alkimos out; remaining out-of-sector employers identified and determined by an admin, row by row. |
| D12 | Suspect merges (`MMA`, `IAS GROUP`, `RIGFORCE`) | Unwind `MMA`; verify the other two | DA1.1 | **Decided, corrected:** IAS (Innovative Asset Solutions, acquired by UGL in 2021) and Rigforce (acquired by Programmed) stay merged, and the re-created rows 827 and 798 are merged in with their names as aliases. Only the `MMA` alias is unwound. No worker was moved by any merge, so the unwind is an alias deletion plus the MMA → Cyan Renewables merge. Amended 2026-09-24 (S12): Rigforce (798) and IAS (827) stay as agreement-holding children of Programmed (28) and UGL (33) with their names as group aliases; not merged. |
| D13 | Confidence vocabulary H/M/L and source vocabulary as in §3.3 | Accept | DA2.1 | **Decided:** accept. |
| D14 | Coordination with the organiser UX programme (WP2.7, WP3.9) on `unit_basis` and the editor | Agree the JSON key and matcher change before WP2.7 ships | DA5.1 | **Decided:** accept. |
...
| D16 | One synthetic worker record (1536, the operator's own test identity: employer TestCo 2, worksite TEST Alpha FPSO) is a member of campaigns 37, 50 and 64 | Keep it as the test identity for the internal campaign 50 | DA0.2 | **Decided (operator, 22 Sep):** keep the record, re-pointed to AWU WA Branch / AWU Head Office before the TestCo rows are deleted; remove its membership of 64; 37 disappears with the dataset. Active workers after DA0.2: 5,085. |
| D17 | Rehearsal clone cadence | A fresh production-shaped clone is created by the operator after Phase 0's run sheets have landed on production (so it carries DA0.3's migration, the synthetic removal and the vessel tables) and re-created at each phase boundary, because the weekly membership batch keeps production moving. Phase 0 rehearses on the 12 September clone, which has the synthetic rows and lacks the vessel tables — the right conditions for DA0.2 and DA0.5. The 12 September clone is retired once the fresh clone's pack counts match production and the organiser UX ledger records the new ref as its realistic data set (WP3.8's clone rehearsal is done and WP2.5 has not started, so nothing is in flight on it). One clone, one owner at a time, recorded in the ledger. | DA1.1 onward | **Decided (operator, 22 Sep):** clone after Phase 0; old clone retired then, on the conditions stated. |
...
| Merging a lineage-A workgroup row into its legal employer widens any campaign universe keyed on that row (a universe of `JADESTONE ENERGY STAG CPF` becomes a universe of Jadestone) | The AND rule with the campaign's worksites keeps membership unchanged where the facility is in the universe (campaign 58 has Stag CPF); the rehearsal reports the membership delta per campaign, and a campaign whose universe has no worksite gets its facility added before the merge |

---

## 8. Standing questions after the 22 September decisions

The structural questions in §4.5 and decisions D0–D14 were answered by the operator on 22 September 2026. What the next adjudication session needs to settle:

1. **D15's one remaining question** — whether the vessel-tracking module has its own repository and migrations, which decides whether DA0.5 captures the DDL here or records the tables as external with guards.
2. **Survivor choices the legal-entity rule does not settle by itself** (employer worksheet): Woodside 13 vs 689; Jadestone 693 vs 695; MODEC 93 vs 703; Solstad 47 vs 717; a Transocean root above Sedco Forex; Technip Energies vs TechnipFMC; whether ISOLOGICS is a KAEFER entity; whether DURATEC is WPF Duratec; APA vs DBNGP.
3. **Worksite families** (worksite worksheet): Ichthys (seven rows for three places), Wheatstone LNG (two), Waitsia (three spellings), the NWS hub and its platforms, Barossa / BW Opal, and whether Pluto 2 is a worksite or a project at Pluto LNG.
4. **The 24 missing assets**: which to create now (Bass Strait, WA Oil, the Varanus satellites, the decommissioning fields) and which to defer as planned or historical rows.
5. **The reports the owner needs first** (§3.8), which fixes the order of Phases 3–5.
6. **Patch allocation logic** (D10), revisited in DA5.2 once patch membership can be measured.
