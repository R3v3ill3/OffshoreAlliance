...
- **Stop and ask the operator** when: a decision the package depends on is open (D15 and D16 are open at the time of writing); a rehearsal shows a membership or placement delta the plan did not predict; a run-sheet step returns something its file did not predict; a merge would move workers between employers that hold different agreements and the worksheet row does not already say so; a package needs a third fix round; anything would touch production, change a Supabase project's role, or read a personal field; the FWC coverage clause contradicts organiser knowledge (queue it, do not overwrite).
...
Two facts about the clone bind Phase 1: it predates the September membership sync, so its employer and worksite rows differ from production's (the lineage-C rows are absent), and it lacks the sixteen vessel-tracking tables. Ask the operator for a refreshed production-shaped clone before DA1.1 (see "Operator inputs"); until then rehearse Phase 0 packages on the existing clone and report the difference.
...
|---|---|---|
| Decisions D15 (capture the vessel-tracking tables as a migration now?) and D16 (keep worker 1536 by re-pointing, or delete) | DA0.5, DA0.2 | Put both in the first message. |
| A refreshed production-shaped clone with the September data, including the sixteen vessel-tracking tables | DA1.1 onward | The agent never clones production. Until it exists, Phase 0 rehearses on the existing clone and reports the difference. |
...
**DA0.2 Remove the synthetic dataset.** Fable planner and implementer at `max`; Fable reviewer; Sonnet verifier on the clone. Scope is fixed by D4: employers 787, 791, 788–790, 792–794; worksites 196–199; campaigns 15 and 37; program 6; projects 18–21; 11 employer–worksite roles; 665 workers. Deletion order from plan §5 (campaigns → workers → projects → program → worksites → employers); the preflight lists child-row counts per table for the 665 workers and the disposition of worker 1536 per D16. Rehearse forward-back-forward on the clone (the clone has the same synthetic rows). Acceptance: active workers on production fall by 665 (664 if 1536 is kept), `05` shows no `test` cluster, campaigns 15 and 37 gone. Depends on D16.
...
**DA0.5 Vessel-tracking schema into the ledger.** Opus planner; Fable implementer; Fable reviewer. Derive the DDL of `vessels`, `geofences` and the fourteen `mobilisation_*` tables read-only from `pg_catalog` through the connector (never by linking the CLI to production), write it as a migration that is idempotent where the objects already exist, rehearse on the clone (which lacks them), regenerate types from the clone, and reconcile `vessels.owner_operator_id` and `mobilisation_watch_contractors.employer_id` against the employer register in the ledger's incidental findings. Depends on D15.

Phase 0 exit: DA0.2, DA0.3 and DA0.5 on production; the pack shows 5,084 (or 5,085) active workers and no new lineage-C rows after the next weekly membership batch.
