...
| DA0.2 | Remove the synthetic dataset (D4, confirmed): employers 787, 791 and the six same-day contractors 788–790, 792–794; worksites 196–199; campaigns 15 and 37; program 6; projects 18–21; 11 employer–worksite roles; 665 workers (none with a membership number) and their cascaded rows. Order: campaigns → workers → projects → program → worksites → employers, because `workers.employer_id`, `workers.worksite_id` and `projects.worksite_id` are `NO ACTION` while the campaign and role junctions cascade. The preflight lists child-row counts per table (SMS, email, call and rating rows attached to the 665) and worker 1536 — the operator's own test record, a member of campaigns 37, 50 and 64 (D16) — which is re-pointed or deleted per that decision before the rows it depends on go. Rehearse on the clone with a rollback; then the operator runs it on production | `00_preflight.sql`, `10_remove_test_dataset.sql`, `90_rollback.sql` | active workers fall by 665 to 5,084; `05_candidate_clusters.sql` no longer lists a `test` cluster; campaigns 15 and 37 gone |
...
| DA0.5 | Vessel-tracking schema into the ledger (D15): capture `vessels`, `geofences` and the `mobilisation_*` tables as a migration (`supabase db diff` against production), regenerate types, confirm the clone carries them; reconcile `mobilisation_watch_contractors.employer_id` and `vessels.owner_operator_id` against the employer register | migration | `pnpm validate:migrations` green; the clone has the 16 tables; every `owner_operator_id` and watch-contractor `employer_id` resolves to a canonical employer |
...
| D3 | Q-S2: restore collapsed agreement-holding subsidiaries | Accept for Monadelphous and Programmed; others case by case | DA1.1 | **Decided:** as recommended. |
...
| D15 | Sixteen vessel-tracking tables exist in production outside the migration ledger (§1.11). Are they captured as a migration before Phase 2 references `vessels`? | Capture with `supabase db diff`; link `worksites.vessel_id`; fold the watch-contractor aliases into `employer_name_aliases` | DA0.5, DA1.2 | **Context given (operator, 22 Sep):** a separate development piece built to alert on vessels heading towards the relevant fields during the construction and decommissioning lead time. The migration capture, the `worksites.vessel_id` link and the alias fold-in remain the recommended actions; not yet ruled on. |
| D16 | One synthetic worker record (1536, the operator's own test identity: employer TestCo 2, worksite TEST Alpha FPSO) is a member of campaigns 37, 50 and 64. Keep or delete? | Keep it as the test identity for the internal campaign 50: re-point its employer and worksite to that campaign's universe (AWU WA Branch, AWU Head Office) before the TestCo rows are deleted; remove its membership of 64 (37 goes with the dataset); it has no placements or ratings in 64 | DA0.2 | **Identified 22 Sep;** keep-by-re-pointing vs delete awaiting the operator |
...
1. **D15 and D16** — whether the vessel-tracking tables are captured as a migration now (DA0.5); whether the operator's test record 1536 is kept for campaign 50 by re-pointing, or deleted with the dataset.
