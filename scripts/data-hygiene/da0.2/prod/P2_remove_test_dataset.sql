-- DA0.2 PRODUCTION RUN SHEET — step P2 (mutating: removes the synthetic dataset, one transaction)
...
-- What it does: re-points worker 1536 to AWU WA Branch (741) / AWU Head Office (185) and out of campaign 64; deletes campaigns 15 and 37, the 664 synthetic workers, projects 18–21, program 6, worksites 196–199 and employers 787–794 with every child row, logging every row to public._oux_hygiene_log; asserts every other campaign's membership and placement checksums unchanged. Takes SHARE ROW EXCLUSIVE locks on workers, campaign_worker_membership and campaign_worker_ou for the transaction (run in a quiet window).
-- Expect: workers_active 5085, workers_total 5900, campaigns_15_37 0, employers_787_794 0, worksites_196_199 0, roles 0, w1536 'emp=741 ws=185 cwm=50', test_worksite_cluster 0, log_rows_pending 3139, snapshot_rows_logged 3138.
-- Paste back: the one result row (or the error). Anything else than the expected row: run PX1 and report.
