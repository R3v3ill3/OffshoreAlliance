...
| DA2.1 | Provenance columns and vocabularies (`fact_reviews`) — runs before DA1.1 | planning (Opus planner, 2026-09-24) | `claude/determined-hypatia-y2cqau` | | | Lands on the fresh clone first (D17); the clone does not exist yet | D13 |
| DA1.1 | Employers round | planning: analyst preparing the round (Opus, 2026-09-24) → adjudication session with the operator → Fable planner/implementer | `claude/determined-hypatia-y2cqau` | | | Merges change campaign universes (plan §7); every affected campaign listed before the operator sees a run sheet | D2, D3, D11, D12 |
| DA1.3 | Scope taxonomy | planning (Opus planner, 2026-09-24); independent of DA1.1 | `claude/determined-hypatia-y2cqau` | | | Readers of `work_scopes` that assume a flat list | D7 |
