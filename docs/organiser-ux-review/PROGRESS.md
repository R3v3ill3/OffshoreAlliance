...
| 2.1 | Schema and migration | reviewer **APPROVE FOR PR/MAIN WITH PRODUCTION DB GATE**; ready evidence commit/PR; clone green; normal-dev schema/e2e waived | `feat/oux-wp2.1-schema-migration` | pending | Clone types exit 0 (expected WP2.1 symbols plus harmless ordering swap of two `user_profiles_organiser_id_fkey` entries); tsc exit 0; migration validation 10; F1 37/37; full tests 83/1,118; touched ESLint exit 0; lint baseline 294 (143/151); Next 16 build exit 0 / 130 routes. Thin-dev exact read-only `00`: units 8, memberships 111, placements 111, F1/H10 residual 0; not integrated. | Production DB gate: current `00` with `malformed_or_nonpositive_basis_units = 0`, reviewed mappings, cleanup, then atomic `db push`/`psql -1`; no plain migration `psql -f`. Explicit dev/e2e gap; recovered cleanup bytes not re-executed; 25P01 retained; exact whole-file rollback-only `95` has no marker. WP2.2 owns Insert typing, unpaged loader, API/enforcement/view/M2/Recompute before any group-model consumer | 3, 4, 5; E2/M2/C1/F1 |
...
| WP0.4 production run (2026-09-11) | Production has 193 historical rule-sourced placements with `assigned_rule_id IS NULL` across campaigns 42, 57 and 64. Five same-group/type worker partitions contain 29 excess placements; active rule definitions are only on campaigns 23 and 58, so the current 03 guard skips the remaining duplicates even though no live rule can recreate them. One eligible duplicate was safely removed. Cross-campaign membership is unaffected and remains supported. | **Pre-WP2.1 data fix.** Preserve one unit per group per campaign using current worker dimensions/manual intent; worker 3014's campaign-57 worksite is ambiguous and requires operator input |
| WP2.3 final review (2026-09-11) | Dropping on a nested sub-unit card bubbles to its parent and can call `moveWorkers.mutate` twice; the current no-op guard usually masks the second call. Characterisation pins the existing behaviour so WP2.3 does not change it. | Unassigned separate behaviour fix; do not fold into the decomposition |
