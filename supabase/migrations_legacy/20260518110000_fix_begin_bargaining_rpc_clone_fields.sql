-- ============================================================
-- Migration 20260518110000: Fix begin_bargaining_phase RPC —
-- include bargaining context fields in the SA clone
--
-- The original RPC at 20260516100000 omitted all Phase 2
-- bargaining context columns from the situation-analysis clone
-- INSERT (nerr_status, nerr_issued_at, bargaining_phase_state,
-- employer_ballot_intent, prior_employer_ballots, key_disputes,
-- worker_support_estimate). These fields are silently dropped when
-- the Phase 2 wizard runs in continuation mode, discarding data
-- the organiser entered in wizard step 7.
-- ============================================================
...
BEGIN
  ...
  -- Includes all bargaining context fields (Phase 2 extension columns).
  IF p_carryforward_situation_analysis_id IS NOT NULL THEN
    ...
    INSERT INTO campaign_situation_analyses (
      ...
      ai_model,
      ai_prompt_snapshot,
      -- Bargaining context columns (previously omitted — now carried forward)
      nerr_status,
      nerr_issued_at,
    ...
    SELECT
      ...
      ai_model,
      ai_prompt_snapshot,
      -- Bargaining context overrides
      COALESCE(
        p_situation_overrides->>'nerr_status',
        nerr_status
      ),
      COALESCE(
        (p_situation_overrides->>'nerr_issued_at')::DATE,
        nerr_issued_at
      ),
      COALESCE(
        p_situation_overrides->>'bargaining_phase_state',
        bargaining_phase_state
      ),
      COALESCE(
        p_situation_overrides->>'employer_ballot_intent',
        employer_ballot_intent
      ),
      CASE WHEN p_situation_overrides ? 'prior_employer_ballots'
           THEN p_situation_overrides->'prior_employer_ballots'
           ELSE prior_employer_ballots END,
      CASE WHEN p_situation_overrides ? 'key_disputes'
           THEN p_situation_overrides->'key_disputes'
           ELSE key_disputes END,
      CASE WHEN p_situation_overrides ? 'worker_support_estimate'
           THEN p_situation_overrides->'worker_support_estimate'
           ELSE worker_support_estimate END
    FROM campaign_situation_analyses
...
COMMENT ON FUNCTION begin_bargaining_phase IS
  'Atomically begins the Bargaining to Win phase for a campaign. Creates stage plans 7-11, carries forward ambitions, clones the situation analysis (including all Phase 2 bargaining context fields) with overrides, seeds the intractable tracker and PIA pack. Returns summary JSONB with stage_7_plan_id.';
