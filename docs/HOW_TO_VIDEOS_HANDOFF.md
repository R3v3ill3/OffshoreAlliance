...
**C‑series scroll fix (2026‑06‑10, commit `fix(help): wall-chart clips (C1-C3) now scroll…`):** the wall‑chart clips originally stayed pinned at the top (campaign summary + assessment distribution) so the unit cards / colour‑coded worker tiles were never on screen. Root cause: `moveTo()`/`ctx.move()` only move the synthetic cursor to an element's current box — they do **not** scroll. Fix: a `showTiles(page)` helper (`[data-worker-id]` first tile → `scrollIntoViewIfNeeded()` + `scrollIntoView({block:'center'})`) called before the tile cursor‑work; for top controls (assessment selector) a `showTop()` scrolls back up. **C3 also switched its drag to a manual pointer drag** (`mouse.down` → small nudge to arm the dnd‑kit sensor → move to "Drop worker tiles" → `mouse.up`) since Playwright's HTML5 `dragTo()` doesn't trigger dnd‑kit — a worker now actually lands in the build list. **Lesson for any future wall‑chart clip: always scroll tiles into view; tiles are below the summary/distribution blocks.** Media re‑uploaded to both buckets.
