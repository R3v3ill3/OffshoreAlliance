...
| 12 | Shared assessments (ASC, RAT, VIEW) | ASC-a `campaign_activities.scope` (`campaign` \| `family`); RAT-a a rating recorded in a child lands on the parent's activity row; VIEW-a a child sees a shared activity's ratings only for its own members. See below. | WP3.8, WP3.9 | Open |
...
Raised by the operator from campaign 64 (`PROGRESS.md` programme backlog, 2026-09-17) and planned as WP3.8 (schema: parent link and assessment scope) and WP3.9 (UI: "Start a campaign from this unit"), whose specifications are appended to `IMPLEMENTATION_ORCHESTRATION_PROMPT.md` phase 3. Labels are unique programme-wide. "As recommended" is a complete answer. **Status: Open — awaiting the operator.**
