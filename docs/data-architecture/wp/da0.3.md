...
| P3 | `prod/P3_preflight_after.sql` | **matched**: 05:48 UTC; employers/worksites counts and md5s identical to P1; all four view md5s identical; name_match_reviews, fold_name(text), decide_name_match(jsonb), name_match_reviews_fold() present; 2 triggers, 1 policy, 7 indexes; raw columns present, 0 set; both CHECKs merge/manual/import/oa_universe/fwc; ledger_row_present 1. **Migration on production 2026-09-24; next: merge the branch (promotion gate satisfied), then the weekly batch through the new path.** | |
