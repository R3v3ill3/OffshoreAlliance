...
Production holds 264 tables in `public`. Sixteen of them appear in no file under `supabase/migrations/`, `supabase/migrations_legacy/` or `supabase/reconciliation/`, and nothing under `apps/` or `packages/` reads them: `vessels`, `geofences` and fourteen `mobilisation_*` tables (`sources`, `signals`, `rules`, `alerts`, `alert_signals`, `alert_events`, `notifications`, `positions`, `prefs`, `push_subscriptions`, `settings`, `watch_contractors`, `watch_keywords`, `watch_vessels`). They are the vessel-tracking module the context note mentions, and they are live: `vessels` holds 25 rows with IMO, MMSI, name, type, owner name, `owner_operator_id` (a foreign key to `employers`), relevance, last AIS position and geofence membership; `mobilisation_watch_contractors` holds 14 rows with a `canonical_name`, an `aliases` array, an `employer_id` and a `tier`; 19 signals and 18 alerts have been recorded, several keyed to `contractor_id`, `operator_id` and `worksite_id`.
...
| D15 | Sixteen vessel-tracking tables exist in production outside the migration ledger (§1.11). How were they applied, and are they captured as a migration before Phase 2 references `vessels`? | Capture with `supabase db diff`; link `worksites.vessel_id`; fold the watch-contractor aliases into `employer_name_aliases` | DA0.5, DA1.2 | Open |
| D16 | Two synthetic workers are also members of the real campaigns 50 and 64. Delete with the dataset, or re-point? | Inspect by hand in DA0.2's preflight; delete unless they are real people with a wrong employer | DA0.2 | Open |
...
1. **D15 and D16** — how the vessel-tracking tables reached production and their capture as a migration; the two synthetic workers who are also members of campaigns 50 and 64.
