...
|---|---|---|---|
| 0 | DA0.2, DA0.3 and DA0.5 on production; pack shows 5,085 active workers; no new lineage-C rows after the next weekly membership batch | **Run sheets complete on production 2026-09-24**: DA0.2 P1–P3 (5,085 active workers, no `test` cluster), DA0.5 P1–P3 (three ledger rows), DA0.3 P1–P3 (migration, checksums unchanged). Remaining for the exit: merge the branch into `main`; the next weekly membership batch through the new path with 0 new employers/worksites; then the D17 fresh clone | run sheets 2026-09-24; exit pending the merge and the next batch |
...
| Open the pull request from `claude/determined-hypatia-y2cqau` into `main` and merge it (the promotion gate is satisfied: the DA0.3 migration is on production); check the gen-types workflow's regeneration produces no diff | DA0.3 | pending |
