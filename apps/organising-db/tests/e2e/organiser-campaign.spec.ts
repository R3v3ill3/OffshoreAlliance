...
/**
 * The header's back arrow and its heading, as one selector list. CSS
 * selector lists match in document order, so `.first()` on this locator is
 * whichever of the two the DOM puts first — which is how the reading order
 * (back arrow, then heading, then badges) is pinned without a testid.
 */
const HEADER_BACK_OR_HEADING = 'header button[aria-label="Back to campaigns"], header h1';
...
test.describe("Campaign workspace — full mode is today's page", () => {
  ...
  test("the eight tabs, in order, with no organiser-mode furniture", async ({ page }) => {
    ...
    // …and its reading order: back arrow, then the heading. A selector list
    // returns elements in document order, so `.first()` is the DOM's first.
    await expect(page.locator(HEADER_BACK_OR_HEADING).first()).toHaveAttribute(
      "aria-label",
      "Back to campaigns"
    );
...
test.describe("Campaign workspace — the organiser-mode round trip", () => {
  ...
  // trips. The default 30s was never enough — it only looked like it was
  // because the run used to abort at the Build list assertion before
  // reaching the last two steps.
  test.slow();

  // Describe-scoped so `afterEach` can reach them without the test body
  // having to hand them over: the reset has to run when the test times out,
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    // skipped suite, this must not try to open an admin storage state that
  ...
  test("four tabs plus More, deep links, the switcher and every header action", async ({
    ...
    expect(admin, "beforeAll must have opened the admin context").toBeTruthy();
    ...
    // 1b — the header's reading order and its single heading. In organiser
    // mode the switcher *is* the <h1>, and it must follow the back arrow.
    await expect(page.locator(HEADER_BACK_OR_HEADING).first()).toHaveAttribute(
      "aria-label",
      "Back to campaigns"
    );
    ...
    await expect(menu.getByRole("menuitem", { name: /Pending review/ })).toHaveAttribute(
      "aria-disabled",
      "true"
    );
