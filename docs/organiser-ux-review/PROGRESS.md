...
| 2.3 | Wall chart decomposition | approved; final evidence commit and PR pending | `feat/oux-wp2.3-wall-chart-decomposition` | | Local: 83 files / 1093 tests, tsc/build green, lint unchanged at 294 and touched files clean; 2,635-line shell reduced to 320 lines across focused hooks/components; frozen characterisation tests pass unchanged against base and decomposition. Preview `1694b8a`: focused e2e 8/8, full e2e 15 passed / 2 established skips / 0 failed; request graph unchanged; fresh terminal reviewer **APPROVE WITH ADVISORIES** after the operator-authorised third test-only round. Operator accepted the measured +0.237% local raw / +0.21% browser-transfer variance on 2026-09-11 | Fake PostgREST harness does not emulate projection/predicate/order semantics (source/query review + real-backend preview cover it). Existing nested-card drop bubbling invokes the move handler twice and is pinned, not fixed | — |
