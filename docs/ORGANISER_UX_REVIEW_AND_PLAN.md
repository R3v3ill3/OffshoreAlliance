...
**Addendum, 17 September 2026 — campaign families.** Raised from campaign 64 ("ROV sector wide"), a sector campaign with one Employer unit per company, where one company (Total Marine Technology) moved into bargaining and needed vessel → shift structure that a sector campaign should not carry. Three additions to the model, delivered by WP3.8 and WP3.9 (`docs/organiser-ux-review/IMPLEMENTATION_ORCHESTRATION_PROMPT.md` phase 3; decisions 11 and 12 in `DECISIONS.md`):

- **A one-level parent link.** A campaign may be *part of* one parent campaign (`campaigns.parent_campaign_id`); a campaign with children cannot itself have a parent, and the database enforces both. The link changes nothing about membership, groups, units or placements: a child's membership still comes from its own universe, and no unit or placement row ever moves or points across campaigns. Cross-campaign membership (a worker in the sector campaign and in their employer's bargaining campaign) is already supported; the link only names the relationship.
- **Shared assessments.** An assessment belongs to one campaign, as today. A parent may mark an assessment as shared with its family (`scope = 'family'`), and every child then reads and rates that one assessment as if it were its own: one activity row, one rating row per worker, never a copy. In a child, the shared assessment's ratings are shown only for the child's members; in the parent, for everyone. A sector petition is rated from the bargaining campaign's chart and counts in the sector campaign automatically; a protected-action ballot stays campaign-specific.
- **Start a campaign from this unit.** From an Employer (or Worksite) unit in a parent campaign, an organiser creates the child campaign through the same three-screen create flow (5.8), prefilled: name, kind, a universe made of the unit's basis, the parent link, the assessments to share, and the Worksite group built from Who's in. Membership and placements in the child are derived by the universe sync; nothing is copied or moved out of the parent, whose own structure is untouched.

**The sector-campaign shape is correct.** A sector campaign's primary grouping is Employer (one unit per company); vessels may be nested under a company as sub-units (on campaign 64 they are custom Work area units, "constructor" and "Normand", placed by hand), which makes that group sub-unit-only and therefore absent from the group selector (WP2.4c, SG-a). That is the intended rendering, not a defect: the sector view is organised by company, Employer × Worksite stays available as Compare, and the vessel → shift depth belongs in the company's own campaign.
