-- ============================================================
-- Unify record_call_attempt overloads
--
-- Before this migration three overloads of record_call_attempt
-- co-existed in public:
--   • 14-arg legacy (pre-cta_ratings) — DEAD, no callers
--   • 17-arg harmonisation (cta_ratings + assessment_ratings + action_id)
--   • 19-arg share-token (cta_ratings + 4 share-token cols)
--
-- The deprecated /api/campaigns/[id]/call-attempts route passes 15
-- keys, which ambiguously matches both the 17-arg and 19-arg — a
-- 300 Multiple Choices error waiting to happen.
--
-- This migration drops the 14-arg and 19-arg overloads and replaces
-- the 17-arg with a single unified 21-arg signature that absorbs the
-- four share-token columns (defaulting to NULL). Every caller now
-- resolves to one function; missing keys fall back to defaults.
-- ============================================================

DROP FUNCTION IF EXISTS record_call_attempt(
  INTEGER, INTEGER, UUID, VARCHAR, VARCHAR, TEXT, TIMESTAMPTZ,
  VARCHAR, TEXT, VARCHAR, INTEGER, JSONB, JSONB, JSONB
);
...
BEGIN
  ...
  -- 1. Insert call_attempt (share-token attribution columns default to
  --    NULL when the caller didn't supply them, so the regular dialler
  --    flow stores NULLs and the share-token flow stores the IDs).
  ...
  -- 3. Determine new item status. 'removed_from_campaign' /
  --    'no_longer_in_universe' are terminal — the worker is being
  --    removed, so the list item is always marked completed.
  ...
  -- 4. Update call_list_items. Always clear the share-token claim
  --    columns — they're NULL for the regular dialler so the clears
  --    are a no-op, and the share-token flow needs them released.
  ...
  -- 7. Insert worker_activity_log (action_id links to phone-call session;
  --    NULL for share-token flow since there's no orchestrator action row).
  ...
  -- 8c. Per-call assessment ratings (multi-assessment capture)
