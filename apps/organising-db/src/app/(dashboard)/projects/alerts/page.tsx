...
export default function MobilisationFeedPage() {
  ...
  const linkedSignalId = useSyncExternalStore(subscribeToHash, hashSignalId, () => null);
  ...
  const filtered = (signals.data ?? []).filter((row) => {
    ...
    if (query.trim()) {
      const hay = `${row.title} ${row.extract ?? ""}`.toLowerCase();
    ...
    <div className="space-y-6">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                ...
                {alert.vessel_id && (
                  <>
                    ...
                    <Link
                      className="underline"
                      href={projectVesselPath(alert.vessel_id)}
                ...
                {alert.watch_contractor_id && (
                  <>
                    ...
                    <Link
                      className="underline"
                      href={projectContractorPath(alert.watch_contractor_id)}
                      onClick={(event) => event.stopPropagation()}
                    >
                      contractor
                    </Link>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <input
          ...
          placeholder="Search title or extract"
        />
        ...
        {filtered.map((row) => (
          ...
          >
            ...
            <div className="flex flex-wrap gap-3 text-xs">
              {row.watch && (
                <Link className="underline" href={projectContractorPath(row.watch_contractor_id!)}>
              ...
              {row.vessels && (
                <Link className="underline" href={projectVesselPath(row.vessel_id!)}>
