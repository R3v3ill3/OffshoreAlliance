...
| 8b | Chat workspace layout | ~~**Member board + focused thread, with a "next member needing a reply" button.** Board tiles adapted from `worker-tile.tsx`; pinned assessment inline in the main column, not the sidebar.~~ **SUPERSEDED 2026-08-17 by 8c.** | §E0 |
| 8c | Chat workspace layout (replaces 8b) | **Three panes: roster rail (left, narrow) · conversation (middle) · member card (right, always visible).** Selecting a name in the rail populates both the middle and right panes. Rail rows are full-row colour-coded by a merged state ladder with a slow orange pulse on new inbound (`prefers-reduced-motion` fallback mandatory); rating shows as a separate saturated `RATING_LEVELS` chip so the two palettes never collide. Openers compose in the middle pane for unmessaged members; bulk waves via a rail Select mode. Pinned assessment and notes live in the right pane, expanded by default. §E0's two objections to the roster rail (density; right rail below `xl`) are answered by a "Needs reply" filter and explicit responsive rules — see `docs/SMS_MODULE_PHASE10_PLAN.md` §0.2. | §E0, `SMS_MODULE_PHASE10_PLAN.md` |
