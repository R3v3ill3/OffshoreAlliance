-- WP3.8 PRODUCTION RUN SHEET — recovery only: reverse P2 (after PX1 if P4 ran)
...
-- What it does: removes everything P2 added and restores the two views to their previous definition. STOPS if any campaign still has a parent or any assessment is still shared. The ledger row is removed by the separate statement at the very end, run only after this file succeeds.
-- Expect: a result row with every *_remains false and summary_checksum_before = summary_checksum_after.
-- Paste back: the result row.
...
-- Separate statement, run on its own ONLY after the rollback above succeeded:
-- DELETE FROM supabase_migrations.schema_migrations WHERE version = '20260917100000';
