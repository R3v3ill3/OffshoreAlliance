...
| DA2.1 | Provenance columns and vocabularies (`fact_reviews`) — runs before DA1.1 | **plan written 2026-09-24** (`wp/da2.1.md`: migration `20260925010000_da2_1_provenance.sql`; employers +11 columns, worksites +17, agreement_worksites +6 with `mapping_confidence` → `confidence`; cycle guards; `fact_reviews` + `record_fact_review()`; 369 worksheet rows to load). Open: O-1 reuse `worksites.operator_id` as the facility operator instead of a new column (orchestrator recommends yes; §3.2 amendment for the operator), O-4 catch dev up with the five missing migrations before rehearsal, O-5 the fresh clone — **Implemented 2026-09-24 (Fable):** migration `20260925010000_da2_1_provenance.sql` (md5 c7ec8395…), `scripts/data-hygiene/da2.1/` with `prod/` P1–P5 + PX1/PX2, fact_reviews builder (369 rows, 162 decided, md5 b595f334…), 22 unit tests; dev forward → back → forward with identical digests; gates at baseline. **Fable review: APPROVE WITH ADVISORIES** (stale prod/ Expect lines → 191 worksites / ledger tail incl. 20260924070000 / max_depth 1; auth.users lock note; `91` decided_at compared as timestamptz) — being applied; next: verifier rehearsal on `OA_clone_2` (apply 20260924070000 there first) | `claude/determined-hypatia-y2cqau` | | | Lands on the fresh clone first (D17); the clone does not exist yet | D13 |
| DA1.1 | Employers round | **adjudicated 2026-09-24** (`wp/da1.1-round.md` §11–§13: S1–S30 answered by the operator in the session; 116 worksheet rows decided; D12 amended, D3 noted; 24 merges, 12 children, 130 keeps, 11 out, 2 placeholders, 18 new rows in three rounds) → next: Fable planner for the merge and coverage scripts (with the `merge_employers` corrective migration) after DA2.1's migration — **2026-09-24 (later): plan `wp/da1.1.md` + corrective migration `20260925120000_da1_1_merge_employers_v2.sql` + scripts `scripts/data-hygiene/da1.1/` written by the Fable planner; Fable review round 1: SEND BACK (alias carry leaves victim aliases so the FK sweep raises on 689/6/826; `30` creates child rows whose names are still alias strings before moving them; writes to rows with an empty worksheet decision (O-2/O-3, 780, 72) — gate added; 9 advisories incl. DA1.3's `work_scopes.code` being a hard prerequisite). Fix round 1 applied; **review round 2: APPROVE WITH ADVISORIES** (aliases_dropped in the merge payload; worksheet-backed decision gate incl. survivors 688/690/691/723; `40_dev_rpc_test.sql` C1–C7; prod/ header note) — all applied; approval conditional on the clone rehearsal after DA2.1 and DA1.3 land there and on the decision lists being filled with the worksheet. Orchestrator answered O-5 (write `organiser` + source_ref), O-6 (RIGFORCE → 798, IAS GROUP → 827), O-7 (defer `ugl` alias to `13`); O-2, O-3, O-8, O-10 and the 780/72 parent decisions put to the operator | `claude/determined-hypatia-y2cqau` | | | Merges change campaign universes (plan §7); every affected campaign listed before the operator sees a run sheet | D2, D3, D11, D12 |
| DA1.3 | Scope taxonomy | **plan written 2026-09-24** (`wp/da1.3.md`: migration `20260925090000_da1_3_scope_taxonomy.sql` adds `work_scopes.code`, the 14 map scopes as top-level rows with existing entries re-parented beneath, `scope_crosswalk`; back-fill of `agreement_scopes` from `sector_id` then `source_sheet` covers 135/136 agreements, 1096 needs a per-agreement addition); awaiting the operator's crosswalk confirmations (bundled with the DA1.1 round) and the fresh clone — **Implementation started 2026-09-24 (Opus)** after DA2.1's migration reached the tree; DA1.1's preflight requires `work_scopes.code`, so DA1.3's migration precedes every DA1.1 data file | `claude/determined-hypatia-y2cqau` | | | Readers of `work_scopes` that assume a flat list | D7 |
...
| DA1.1 review (2026-09-24) | Production's ledger carries `20260924070000_membership_update_review` with its statements stored (applied through the migration API or CLI, not the SQL editor). It is `main`'s newest commit `1c6b70dc` (from branch `claude/relaxed-heisenberg-jvigqn`), not on this branch. Normal dev and `OA_clone_2` lack it: apply it (with its ledger row) to dev and to the clone before the next rehearsal so both match production; merge `main` into this branch before the next PR. Who applied it to production is a question for the operator (D0 forbids agents) | orchestrator |
