-- ============================================================
-- Idempotent re-application of campaign workflow tables.
-- Migration 0013 was never applied to the live database; the
-- merge_employers function (20260331120000) references
-- campaign_employers at runtime and fails with
-- "relation does not exist" without this.
--
-- All statements use IF NOT EXISTS / OR REPLACE / exception
-- handlers so this is safe to run even if some objects already
-- exist.
-- ============================================================

-- 1. Extend campaigns (columns may already exist — IF NOT EXISTS is safe)
...
-- 3. Worker membership in campaign
...
CREATE OR REPLACE TRIGGER trg_campaign_worker_membership_updated_at
...
CREATE OR REPLACE TRIGGER trg_campaign_organising_units_updated_at
...
-- 9. RLS — enable (safe if already enabled) and policies (skip if duplicate)
