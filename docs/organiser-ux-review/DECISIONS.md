...
| 5 | No nesting in the organiser model | Flatten employer → worksite containers into two groups (Employer, Worksite); "Split" creates sibling units in the same group; `parent_ou_id` survives only as migration input. **Amended again 2026-09-15 (operator, live testing of WP2.4 on campaign 42):** nesting *within* a group is required — an employer with worksites as the primary grouping and, for a large worksite, shift sub-units under it. Shifts as a sibling group are wrong (every small-worksite worker shows as "Unassigned in Shift", mixed with the large worksite's workers not yet on a shift). The v2 chart must render child units nested under their parent card inside the parent's group, with the parent's own area holding members not yet in a child; groups made only of sub-units are not offered as a primary group. `parent_ou_id` is therefore a live column again, not migration input. Delivered by WP2.4c. | WP2.1 → WP2.4c | Amended |
...
| 11 | Campaign families (FAM, SET, SPN, UNV, KND, MIR) | FAM-a `campaigns.parent_campaign_id` (one level, enforced by trigger); SET-a a writer of both campaigns sets or clears it from the child's Basics sheet; SPN-a a spin-off rebuilds the child's structure (parent's nested sub-units untouched); UNV-a the child's universe is the source unit's basis only; KND-a Bargaining; MIR-a WP2.4b lands before WP3.9 ships. See "Campaign families and shared assessments" below. | WP3.8, WP3.9 | Confirmed (SPN as SPN-a′) |
| 12 | Shared assessments (ASC, RAT, VIEW) | ASC-a `campaign_activities.scope` (`campaign` \| `family`); RAT-a a rating recorded in a child lands on the parent's activity row; VIEW-a a child sees a shared activity's ratings only for its own members. See below. | WP3.8, WP3.9 | Confirmed |
...
## Campaign families and shared assessments (decisions 11 and 12, raised 2026-09-17)

Raised by the operator from campaign 64 (`PROGRESS.md` programme backlog, 2026-09-17) and planned as WP3.8 (schema: parent link and assessment scope) and WP3.9 (UI: "Start a campaign from this unit"), whose specifications are appended to `IMPLEMENTATION_ORCHESTRATION_PROMPT.md` phase 3. Labels are unique programme-wide. **Status: answered by the operator on 2026-09-17 — all recommendations accepted, with SPN taken as SPN-a′ (see Answers).**

**Decision 11 — campaign families**

| Label | Question | Options | Recommendation |
|---|---|---|---|
...
**Decision 12 — shared assessments**
...
**Facts these decisions rest on (established 2026-09-17; cite, do not re-derive).** `campaign_activities.campaign_id` is NOT NULL (`20260908050000_baseline_schema.sql:8871`); ratings hang off `activity_id` (`:7502`); every reader is per campaign. App readers of `campaign_activities` (17 files, `grep 'from("campaign_activities")' src`): `assessment-selector.tsx:44–50`, `participation-selector.tsx`, `use-participation-predicate.ts`, `clear-ratings-dialog.tsx`, `worker-detail-sheet.tsx:1300–1304`, `campaign-assessments.tsx:160–163` and `:269–272`, `assessments/create-assessment-dialog.tsx`, `task-lists/create-task-list-dialog.tsx`, `activists/woc-meeting-dialog.tsx`, `activists/structure-tests-panel.tsx`, `import/worker-import-wizard.tsx`, `api/campaigns/[id]/participation-import/apply/route.ts:305–313`, `api/campaigns/[id]/role-check/route.ts`, `api/campaigns/[id]/an-actions/route.ts`, `api/worker-import/apply/route.ts`, `api/call-share/[token]/route.ts`, `lib/sms/survey-runtime.ts`. Database readers: `campaign_worker_rating_summary` (`:10699–10760`, joins ratings to membership on `a.campaign_id = m.campaign_id`), `worker_ambition_rating` (`:7710`, per plan), `v_worker_escalation_tier`, `vw_sms_assessment_report`, `vw_sms_chat_session_report`, `call_section_funnel`, `v_section_plan_*`, `worksites_view`. RLS: activities insert/update by role only (`:25923`, `:26299`), select `USING (true)` (`:26945`), delete by `can_write_to_campaign(campaign_id)` (`:27518`); ratings insert/update by role only (`:25927`, `:26303`), delete via the activity's campaign (`:27522–27524`) — so a rating recorded from a child against the parent's activity is already permitted by RLS; nothing routes it there yet. Campaign 64: `campaign_type = political`, `sector_wide = false`, universe = eight employers and no worksites (AND rule reduces to the primary employer); one Employer group, eight units, TMT = unit 653 `{employer_id: 84}`; on production two vessel units nested under 653 — **operator's production query 2026-09-17:** 655 "constructor" (32 placements) and 656 "Normand" (11), both `ou_type = work_area`, group 21 "Work area", `unit_basis {custom: true, dimension: 'work_area', parent_ou_id: 653}`; 653 itself has 56 placements. So the vessels are **custom Work area units placed by hand**, not Worksite units: the F1 matcher never targets them (no employer/worksite key, `sync-campaign-universe.ts:156–162`), which closes operator task 2 with no change, and their placements cannot be re-derived from worker records (on the 12 September data set no TMT worker has a `work_area_id`, and only 5 of 33 sit on a vessel-named worksite). This bears on **SPN**: see the note under Answers. Campaigns 61 "Fugro" and 62 "programmed ROV" are the organic precedent (bargaining campaigns beside the sector campaign, one employer each). The vessel → shift structure cannot live in the sector campaign: `cou_enforce_hierarchy_invariants` (`:1688–1718`) permits a third level only under a group container; adding worksites to 64's universe would flip the AND rule and the membership refresh refuses OR mode (`refresh-campaign-universe-membership.ts:67–79`).

**Answers (operator, 2026-09-17: "happy with all those recommendations", after the SPN-a′ note below was put to them).**
- **Decision 11 — Confirmed:** FAM-a, SET-a, **SPN-a′** (recreate the source unit's sub-units in the child as same-kind, same-name units through the structure API and re-assign the same members with `manual` provenance; the parent's rows untouched; no cross-campaign move), UNV-a, KND-a, MIR-a.
- **Decision 12 — Confirmed:** ASC-a, RAT-a, VIEW-a.

**Record of how the answer was reached.**

- **2026-09-17 — operator task 3 (input to WP3.8's data run sheet, not a decision label):** campaign 64 assessments 88 "AN kickoff form", 89 "Sector zoom interest", 90 "Will ask others to join", 91 "has visa story" and 92 "visa witness" are sector-wide (`family`); 93 "kiwi crew" and 95 "pia outreach aug 31" are company-specific (`campaign`). Operator task 1 done the same day: the Total Marine Technology campaign is **campaign 69** on production, with its two vessel units. The WP3.8 data run sheet therefore links 61, 62 and 69 to 64. Decisions 11 and 12 themselves remain **Open**.
- **2026-09-17 — orchestrator note on SPN, from the task-2 result:** the parent unit's sub-units on campaign 64 are hand-placed custom units, which is the condition the recommendation named for reconsidering SPN-a ("only if the operator reports hand placements … on those sub-units that must survive"). A pure rebuild from Who's in cannot recreate them (UNV-a gives the child no worksite rows, so WP2.7's Build from Who's in is disabled, and the workers' own fields do not carry the vessel). Proposed to the operator as **SPN-a′**: the spin-off recreates each sub-unit of the source unit as a unit of the same kind and name in the child (custom Work area units here) through the structure API and assigns the same workers to them with `placements.assign` (`manual` provenance), for workers who are members of the child; the parent's rows are untouched (no move, no cross-campaign re-parenting), so every non-negotiable rule holds. SPN-b (a transactional move) stays the alternative. Awaiting the operator.
- **2026-09-17 — operator, shape of the TMT campaign:** the vessels are **Worksite units** (roots of the Worksite group), not sub-units — the shape the WP3.9 specification plans for a spin-off. Amendment **WSB** proposed for WP2.7 §9.1 and WP3.9: a Worksite-group unit can be keyed to a catalogue worksite without adding it to Who's in (which under the AND rule would shrink membership), with both keys `{employer_id, worksite_id}` in a single-employer campaign so the F1 matcher ranks it above the employer unit and a drag onto it stamps a blank worker worksite (`stampEmployerWorksiteFromOu`, fill-blanks only, `sync-campaign-universe.ts:781–802`). Today's chart New unit writes no worksite basis (`create-organising-unit-dialog.tsx:377`, `:411`).
...
**Phase 0 exit and SMS testing (2026-09-08).** The operator confirmed the phase-0 exit after PRs #22–#26 merged and authorised phase 1. For any dev SMS test in later phases: the seeded dev worker numbers are dummy values outside the Australian range and will not resolve; the operator has designated one personal mobile number (their own, supplied in the session, not recorded here) as the sole permitted test recipient on dev. No agent sends an SMS from dev without confirming the audience is exactly that number.

**WP1.6 sign-offs (2026-09-09).** The operator confirmed all three: (a) after WP1.6 a `user`-role organiser can create or edit units only on campaigns they are on (organiser, team member, creator or edit grant); admins and lead organisers unaffected; (b) worker moves and imports stop auto-enrolling workers into campaigns the actor cannot write to; the skipped count is reported; a privileged org-wide sync is a possible later package; (c) the operator runs WP1.6's production pre-flight query before the deploy and again before hygiene script 01, and pastes the output. Decision 8's note is satisfied by `created_by` defaulting to the creator.
